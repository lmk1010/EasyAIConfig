...
fn peek_codex_session(path: &Path) -> (u64, String, String, String) {
    ...
    for line in text.lines() {
        count += 1;
        if !first_user.is_empty() && !model.is_empty() && !started_at.is_empty() {
            continue;
        }
    ...
    fn walk(p: &Path, since: u64, counts: &mut HashMap<String, u64>) {
        ...
        for e in entries.flatten() {
            ...
            for line in text.lines().take(20) {
                let Ok(v): Result<Value, _> = serde_json::from_str(line.trim()) else {
                    continue;
                };
        ...
        for entry in entries.flatten() {
            ...
            *total_sessions += 1;
            if let Ok(meta) = entry.metadata() {
                if let Ok(mtime) = meta.modified() {
                    if let Ok(dur) = mtime.duration_since(std::time::UNIX_EPOCH) {
                        if dur.as_secs() >= today_cutoff_epoch {
                            *today_sessions += 1;
                        }
                    }
                }
            ...
            if let Ok(meta) = entry.metadata() {
                if let Ok(mtime) = meta.modified() {
                    if let Ok(dur) = mtime.duration_since(std::time::UNIX_EPOCH) {
                        if (dur.as_secs() as i64) < window_start.timestamp() {
                            continue;
                        }
                    }
                }
