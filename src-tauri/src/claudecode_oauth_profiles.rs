...
fn count_running_claude_processes() -> usize {
    #[cfg(unix)]
    {
        ...
        let output = process_command("ps").args(["-axo", "command="]).output();
        ...
        return stdout
            ...
            .filter(|line| {
                ...
                if lower.contains("config-ui") || lower.contains("easyaiconfig") {
                    return false;
                }
                if lower.contains("@anthropic-ai/claude-code") {
                    return true;
                }
                if lower.contains("claude/cli.js") {
                    return true;
                }
    ...
    #[cfg(windows)]
    {
        ...
        return stdout
            ...
            .filter(|line| {
                ...
                if lower.contains("claude\\cli.js") || lower.contains("claude/cli.js") {
                    return true;
                }
...
static KEYCHAIN_CACHE: Mutex<Option<HashMap<String, (Instant, (String, String))>>> =
    Mutex::new(None);
...
pub(crate) fn list_claudecode_oauth_profiles(_query: &Value) -> Result<Value, String> {
    ...
    for (input, meta) in metas {
        ...
        let has_tokens = meta
            ...
            .unwrap_or(false);
        let is_stale =
            !has_tokens && created_at > 0 && (now_secs - created_at) > STALE_THRESHOLD_SECS;
        enriched.push(json!({
          "id": id,
          "name": name,
