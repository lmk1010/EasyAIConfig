...
fn create_backup(paths: &ScopePaths) -> Result<String, String> {
    ...
    write_secret(
        &target_dir.join("config.toml.bak"),
        &read_text(&paths.config_path)?,
    )?;
    write_secret(&target_dir.join(".env.bak"), &read_text(&paths.env_path)?)?;
    if paths.auth_path.is_file() {
        write_secret(
            &target_dir.join("auth.json.bak"),
            &read_text(&paths.auth_path)?,
        )?;
    }
...
#[cfg(target_os = "macos")]
fn first_macos_command_or_bundle(
    command_names: &[&str],
    ...
    if let Some((_, app_path)) = app {
        for executable in executable_names {
            let candidate = Path::new(app_path)
                .join("Contents")
                .join("MacOS")
                .join(executable);
            if candidate.exists() {
...
fn macos_app_profile(id: &str, label: &str, app_names: &[&str]) -> Value {
    ...
    let (app_name, app_path) = app.clone().unwrap_or_else(|| {
        (
            app_names.first().copied().unwrap_or(label).to_string(),
            String::new(),
        )
    });
    json!({
...
pub(crate) fn delete_codex_provider(body: &Value) -> Result<Value, String> {
    ...
    if let Some(obj) = config.as_object_mut() {
        ...
        let was_active =
            obj.get("model_provider").and_then(Value::as_str) == Some(provider_key.as_str());
        if was_active {
            let next_key = obj
                .get("model_providers")
                .and_then(Value::as_object)
...
pub(crate) fn save_config(body: &Value) -> Result<Value, String> {
    ...
    for old_key in obsolete_env_keys.iter() {
        if old_key.is_empty() || old_key == &env_key {
            continue;
        }
...
pub(crate) fn save_raw_config(body: &Value) -> Result<Value, String> {
    ...
    if !auth_json_raw.trim().is_empty() {
        serde_json::from_str::<Value>(&auth_json_raw)
