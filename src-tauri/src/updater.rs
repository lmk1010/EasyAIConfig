...
fn app_update_progress_store() -> &'static Mutex<Value> {
    APP_UPDATE_PROGRESS.get_or_init(|| {
        Mutex::new(json!({
          "status": "idle",
          "message": "",
          "version": "",
...
fn decorate_install_error(error_text: &str) -> String {
    ...
    if lower.contains("signature") || lower.contains("verify") {
        return format!(
            "更新包签名校验失败：{error_text}。请稍后重试，或手动下载最新安装包覆盖安装。"
        );
    }
    ...
    if looks_like_network_issue(error_text) {
        return format!(
            "更新下载失败（网络异常）：{error_text}。请确认网络可访问更新源（R2/GitHub）后重试。"
        );
    }
...
fn updater_repository() -> String {
    let from_env = updater_runtime_var(
        "EASYAICONFIG_GITHUB_REPOSITORY",
        option_env!("EASYAICONFIG_GITHUB_REPOSITORY"),
    );
...
fn updater_public_key() -> String {
    let from_env = updater_runtime_var(
        ...
        option_env!("EASYAICONFIG_UPDATER_PUBLIC_KEY"),
    );
    if !from_env.is_empty() {
        return from_env;
    }
...
fn build_updater(
    ...
    if endpoint.is_empty() || pubkey.is_empty() {
        return Err(
            "自动更新尚未配置：请先在 GitHub Actions Secrets 中设置签名公钥与发布端点。"
                .to_string(),
        );
    }
...
pub(crate) async fn install_app_update(app: tauri::AppHandle) -> Result<Value, String> {
    ...
    if endpoints.is_empty() || updater_public_key().is_empty() {
        let text = "自动更新尚未配置：请先在 GitHub Actions Secrets 中设置签名公钥与发布端点。"
            .to_string();
