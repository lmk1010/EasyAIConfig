...
fn read_remote_panel_credentials_store() -> Result<Value, String> {
  ...
  let mut parsed = serde_json::from_str::<Value>(&raw).unwrap_or_else(|_| json!({ "version": 1, "records": {} }));
...
fn read_remote_panel_credential(tool: &str, provider_key: &str) -> Result<Option<RemotePanelCredential>, String> {
...
fn payload_contains_key(payload: &Value, aliases: &[&str]) -> bool {
  find_number_with_path(payload, aliases).is_some() || find_string_with_path(payload, aliases).is_some()
}
...
fn infer_provider_type(endpoint: &EndpointSpec, payload: &Value, base_url: &str) -> String {
  ...
  if endpoint.panel_hint == "newapi"
    ...
    || payload_contains_key(payload, &["used_quota", "remain_quota", "request_count", "quota"])
  {
...
fn extract_metrics(endpoint: &EndpointSpec, payload: &Value, base_url: &str) -> RemoteUsageAggregate {
  ...
  out.remaining = find_number_with_path(data, &[
    "remaining",
    "remain",
    "remain_quota",
    "remaining_quota",
  ...
  out.used = find_number_with_path(data, &[
    "used",
    "used_quota",
    "used_amount",
    "consumed",
  ...
  out.total = find_number_with_path(data, &[
    "total",
    "total_quota",
    "quota_limit",
    "hard_limit",
  ...
  out.cost = find_number_with_path(data, &[
    "cost",
    "total_cost",
    "total_cost_usd",
    "amount",
  ...
  out.tokens = find_number_with_path(data, &[
    "total_tokens",
    "total_token",
    "tokens",
    "token_count",
  ...
  out.requests = find_number_with_path(data, &[
    "request_count",
    "requests",
    "request_total",
    "total_requests",
...
fn merge_metrics(target: &mut RemoteUsageAggregate, next: RemoteUsageAggregate, endpoint_id: &str) {
  ...
  if !target.source_endpoints.iter().any(|item| item == endpoint_id) {
...
fn push_endpoint(
  ...
  endpoints.push(EndpointSpec { id, label, panel_hint, url });
}
...
fn candidate_endpoints(base_url: &str, days: u64) -> Vec<EndpointSpec> {
  ...
  let usage_query = format!("start_date={}&end_date={}", start_date.format("%Y-%m-%d"), end_date.format("%Y-%m-%d"));

  let mut endpoints = Vec::new();
  push_endpoint(&mut endpoints, "newapi_user_self", "NewAPI 用户余额", "newapi", join_url(&root, "/api/user/self"));
  push_endpoint(&mut endpoints, "newapi_token_self", "NewAPI Token 余额", "newapi", join_url(&root, "/api/token/self"));
  ...
  for billing_base in billing_bases {
    ...
    push_endpoint(
      ...
      "billing",
      format!("{}?{}", join_url(&billing_base, "/dashboard/billing/usage"), usage_query),
    );
  }
...
fn build_headers(api_key: &str) -> Result<HeaderMap, String> {
  ...
  headers.insert(USER_AGENT, HeaderValue::from_static("EasyAIConfig/1.0 relay-usage-check"));
...
fn build_panel_headers() -> HeaderMap {
  ...
  headers.insert(ACCEPT, HeaderValue::from_static("application/json, text/plain, */*"));
  headers.insert(CONTENT_TYPE, HeaderValue::from_static("application/json"));
  headers.insert(USER_AGENT, HeaderValue::from_static("EasyAIConfig/1.0 relay-panel-usage"));
  headers.insert(CACHE_CONTROL, HeaderValue::from_static("no-cache"));
  headers.insert(PRAGMA, HeaderValue::from_static("no-cache"));
  headers
}
...
fn build_newapi_access_headers(access_token: &str, user_id: &str) -> Result<HeaderMap, String> {
  ...
  headers.insert(
    AUTHORIZATION,
    HeaderValue::from_str(&format!("Bearer {}", access_token.trim())).map_err(|error| error.to_string())?,
  );
...
fn set_cookie_header_from_response(headers: &HeaderMap, target: &mut HeaderMap) -> Result<bool, String> {
  let cookies = headers
    .get_all(SET_COOKIE)
    .iter()
  ...
  target.insert(COOKIE, HeaderValue::from_str(&cookies).map_err(|error| error.to_string())?);
  Ok(true)
}
...
fn parse_wrapped_data(text: &str) -> Result<Value, String> {
  ...
  if payload.get("success").and_then(Value::as_bool) == Some(false) {
    ...
    return Err(if message.is_empty() { "upstream returned success=false".to_string() } else { message });
  }
...
fn looks_like_cloudflare_block(status_code: u16, headers: &HeaderMap, body: &str) -> bool {
  ...
  cf_mitigated.contains("challenge")
    || ((status_code == 403 || status_code == 503 || status_code == 429)
      && challenge_text
      && (server.contains("cloudflare") || content_type.contains("text/html") || text.contains("cloudflare")))
}
...
fn attempt_json(endpoint: &EndpointSpec, status: &str, status_code: Option<u16>, message: &str) -> Value {
  json!({
...
fn aggregate_to_json(
  ...
  let provider_type = if aggregate.provider_type.is_empty() { "unknown" } else { &aggregate.provider_type };
  json!({
    "status": "ok",
    "supported": true,
...
fn attach_remote_auth_info(result: &mut Value, credential: &RemotePanelCredential) {
  if let Some(map) = result.as_object_mut() {
    map.insert("panelUrl".to_string(), json!(management_root(&credential.base_url)));
    map.insert("auth".to_string(), json!({
...
async fn panel_get_text(client: &Client, url: &str, headers: HeaderMap) -> Result<(u16, HeaderMap, String), String> {
  let response = client
    .get(url)
    .headers(headers)
    .send()
...
fn panel_attempt(endpoint: &EndpointSpec, status: &str, status_code: Option<u16>, message: &str) -> Value {
...
async fn query_newapi_panel_usage(
  ...
  if credential.auth_mode == "newapi_password" {
    ...
    let (status_code, response_headers, text) = match panel_post_json(
      ...
      Err(error) => {
        ...
        return Ok(remote_error_json("auth_error", "newapi", &credential.provider_key, &credential.base_url, "NewAPI 面板登录请求失败。", attempts));
      }
    };
    if looks_like_cloudflare_block(status_code, &response_headers, &text) {
      attempts.push(panel_attempt(&endpoint, "blocked", Some(status_code), "Cloudflare/bot challenge"));
      return Ok(remote_error_json("blocked", "cloudflare-protected", &credential.provider_key, &credential.base_url, "该中转面板存在 Cloudflare/bot 拦截，当前不支持自动查询余额/远程用量。", attempts));
    }
    if status_code != 200 {
      attempts.push(panel_attempt(&endpoint, "auth_error", Some(status_code), &short_body_message(&text)));
      return Ok(remote_error_json("auth_error", "newapi", &credential.provider_key, &credential.base_url, "NewAPI 用户名/密码登录失败。", attempts));
    }
    let data = match newapi_login_payload_data(&text) {
      Ok(data) => data,
      Err(error) => {
        attempts.push(panel_attempt(&endpoint, "auth_error", Some(status_code), &error));
        return Ok(remote_error_json("auth_error", "newapi", &credential.provider_key, &credential.base_url, &error, attempts));
      }
    };
    ...
    if !login_access_token.is_empty() {
      headers.insert(
        AUTHORIZATION,
        HeaderValue::from_str(&format!("Bearer {login_access_token}")).map_err(|error| error.to_string())?,
      );
    }
    ...
    if !cookie_ok && login_access_token.is_empty() {
      attempts.push(panel_attempt(&endpoint, "auth_error", Some(status_code), "登录成功但未返回会话 Cookie 或 access_token"));
      return Ok(remote_error_json("auth_error", "newapi", &credential.provider_key, &credential.base_url, "NewAPI 登录成功但未返回可用于查询的会话凭据。", attempts));
    }
    attempts.push(panel_attempt(&endpoint, "ok", Some(status_code), "已登录并取得面板会话"));
  } else {
  ...
  let (status_code, response_headers, text) = match panel_get_text(client, &user_endpoint.url, headers.clone()).await {
    Ok(output) => output,
    Err(error) => {
      attempts.push(panel_attempt(&user_endpoint, "network_error", None, &error));
      return Ok(remote_error_json("auth_error", "newapi", &credential.provider_key, &credential.base_url, "NewAPI 用户信息查询失败。", attempts));
    }
  };
  if looks_like_cloudflare_block(status_code, &response_headers, &text) {
    attempts.push(panel_attempt(&user_endpoint, "blocked", Some(status_code), "Cloudflare/bot challenge"));
    return Ok(remote_error_json("blocked", "cloudflare-protected", &credential.provider_key, &credential.base_url, "该中转面板存在 Cloudflare/bot 拦截，当前不支持自动查询余额/远程用量。", attempts));
  }
  if status_code == 401 || status_code == 403 {
    attempts.push(panel_attempt(&user_endpoint, "auth_error", Some(status_code), "用户凭据无权访问 /api/user/self"));
    return Ok(remote_error_json("auth_error", "newapi", &credential.provider_key, &credential.base_url, "NewAPI 面板凭据无权访问用户余额接口。", attempts));
  }
  if status_code != 200 {
    attempts.push(panel_attempt(&user_endpoint, "http_error", Some(status_code), &short_body_message(&text)));
    return Ok(remote_error_json("unsupported", "newapi", &credential.provider_key, &credential.base_url, "NewAPI 用户余额接口返回异常。", attempts));
  }
  let user_data = match parse_wrapped_data(&text) {
    Ok(data) => data,
    Err(error) => {
      attempts.push(panel_attempt(&user_endpoint, "parse_error", Some(status_code), &error));
      return Ok(remote_error_json("unsupported", "newapi", &credential.provider_key, &credential.base_url, "NewAPI 用户余额响应无法解析。", attempts));
    }
  };
  attempts.push(panel_attempt(&user_endpoint, "ok", Some(status_code), "已读取用户 quota"));
  ...
  let mut aggregate = RemoteUsageAggregate {
    ...
    remaining: Some(NumericField { value: quota / NEWAPI_QUOTA_UNITS_PER_RMB, path: "data.quota / 500000".to_string() }),
    used: Some(NumericField { value: used_quota / NEWAPI_QUOTA_UNITS_PER_RMB, path: "data.used_quota / 500000".to_string() }),
    total: Some(NumericField { value: (quota + used_quota) / NEWAPI_QUOTA_UNITS_PER_RMB, path: "(data.quota + data.used_quota) / 500000".to_string() }),
    requests: request_count.map(|value| NumericField { value, path: "data.request_count".to_string() }),
  ...
  if let Ok((stat_code, stat_headers, stat_text)) = panel_get_text(client, &stat_endpoint.url, headers).await {
    if looks_like_cloudflare_block(stat_code, &stat_headers, &stat_text) {
      attempts.push(panel_attempt(&stat_endpoint, "blocked", Some(stat_code), "Cloudflare/bot challenge"));
    } else if stat_code == 200 {
      match parse_wrapped_data(&stat_text) {
        Ok(stat_data) => {
          if let Some(stat_quota) = value_number_any(&stat_data, &["quota"]) {
            aggregate.cost = Some(NumericField { value: stat_quota / NEWAPI_QUOTA_UNITS_PER_RMB, path: "data.quota / 500000".to_string() });
            aggregate.source_endpoints.push("newapi_log_self_stat".to_string());
          }
          attempts.push(panel_attempt(&stat_endpoint, "ok", Some(stat_code), "已读取用户统计"));
        }
        Err(error) => attempts.push(panel_attempt(&stat_endpoint, "parse_error", Some(stat_code), &error)),
      }
    } else {
      attempts.push(panel_attempt(&stat_endpoint, if stat_code == 401 || stat_code == 403 { "auth_error" } else { "http_error" }, Some(stat_code), &short_body_message(&stat_text)));
    }
  }

  let mut result = aggregate_to_json(&mut aggregate, &credential.base_url, &credential.provider_key, days, attempts);
...
fn sub2api_subscription_remaining_usd(data: &Value) -> Option<f64> {
  ...
  for item in items {
    ...
    for key in ["daily", "weekly", "monthly"] {
      let Some(window) = progress.get(key) else { continue; };
      let limit = value_number_any(window, &["limit_usd"]).unwrap_or(0.0);
      if limit <= 0.0 {
        continue;
      }
      let remaining = value_number_any(window, &["remaining_usd"]).unwrap_or(0.0).max(0.0);
...
async fn query_sub2api_panel_usage(
  ...
  let email = if credential.email.is_empty() { &credential.username } else { &credential.email };
  let (status_code, response_headers, text) = match panel_post_json(
    client,
    &login_endpoint.url,
    build_panel_headers(),
    ...
    Ok(output) => output,
    Err(error) => {
      attempts.push(panel_attempt(&login_endpoint, "network_error", None, &error));
      return Ok(remote_error_json("auth_error", "sub2api", &credential.provider_key, &credential.base_url, "sub2api 登录请求失败。", attempts));
    }
  };
  if looks_like_cloudflare_block(status_code, &response_headers, &text) {
    attempts.push(panel_attempt(&login_endpoint, "blocked", Some(status_code), "Cloudflare/bot challenge"));
    return Ok(remote_error_json("blocked", "cloudflare-protected", &credential.provider_key, &credential.base_url, "该中转面板存在 Cloudflare/bot 拦截，当前不支持自动查询余额/远程用量。", attempts));
  }
  if status_code != 200 {
    attempts.push(panel_attempt(&login_endpoint, "auth_error", Some(status_code), &short_body_message(&text)));
    return Ok(remote_error_json("auth_error", "sub2api", &credential.provider_key, &credential.base_url, "sub2api 用户名/密码登录失败。", attempts));
  }
  let data = match parse_wrapped_data(&text) {
    Ok(data) => data,
    Err(error) => {
      attempts.push(panel_attempt(&login_endpoint, "auth_error", Some(status_code), &error));
      return Ok(remote_error_json("auth_error", "sub2api", &credential.provider_key, &credential.base_url, &error, attempts));
    }
  };
  if data.get("requires_2fa").and_then(Value::as_bool).unwrap_or(false) {
    attempts.push(panel_attempt(&login_endpoint, "auth_error", Some(status_code), "账号启用了 2FA，当前不支持"));
    return Ok(remote_error_json("auth_error", "sub2api", &credential.provider_key, &credential.base_url, "sub2api 账号启用了 2FA，当前不支持自动登录查询。", attempts));
  }
  let access_token = value_string_any(&data, &["access_token", "accessToken"]);
  if access_token.is_empty() {
    attempts.push(panel_attempt(&login_endpoint, "auth_error", Some(status_code), "登录响应没有 access_token"));
    return Ok(remote_error_json("auth_error", "sub2api", &credential.provider_key, &credential.base_url, "sub2api 登录成功但没有返回 access_token。", attempts));
  }
  attempts.push(panel_attempt(&login_endpoint, "ok", Some(status_code), "已取得 JWT"));
  let mut headers = build_panel_headers();
  headers.insert(
    AUTHORIZATION,
    HeaderValue::from_str(&format!("Bearer {access_token}")).map_err(|error| error.to_string())?,
  );
  ...
  let (profile_code, profile_headers, profile_text) = match panel_get_text(client, &profile_endpoint.url, headers.clone()).await {
    Ok(output) => output,
    Err(error) => {
      attempts.push(panel_attempt(&profile_endpoint, "network_error", None, &error));
      return Ok(remote_error_json("auth_error", "sub2api", &credential.provider_key, &credential.base_url, "sub2api profile 查询失败。", attempts));
    }
  };
  if looks_like_cloudflare_block(profile_code, &profile_headers, &profile_text) {
    attempts.push(panel_attempt(&profile_endpoint, "blocked", Some(profile_code), "Cloudflare/bot challenge"));
    return Ok(remote_error_json("blocked", "cloudflare-protected", &credential.provider_key, &credential.base_url, "该中转面板存在 Cloudflare/bot 拦截，当前不支持自动查询余额/远程用量。", attempts));
  }
  if profile_code != 200 {
    attempts.push(panel_attempt(&profile_endpoint, "http_error", Some(profile_code), &short_body_message(&profile_text)));
    return Ok(remote_error_json("unsupported", "sub2api", &credential.provider_key, &credential.base_url, "sub2api profile 接口返回异常。", attempts));
  }
  let profile_data = match parse_wrapped_data(&profile_text) {
    Ok(data) => data,
    Err(error) => {
      attempts.push(panel_attempt(&profile_endpoint, "parse_error", Some(profile_code), &error));
      return Ok(remote_error_json("unsupported", "sub2api", &credential.provider_key, &credential.base_url, "sub2api profile 响应无法解析。", attempts));
    }
  };
  attempts.push(panel_attempt(&profile_endpoint, "ok", Some(profile_code), "已读取账户余额"));
  ...
  if let Ok((progress_code, progress_headers, progress_text)) = panel_get_text(client, &progress_endpoint.url, headers.clone()).await {
    if looks_like_cloudflare_block(progress_code, &progress_headers, &progress_text) {
      attempts.push(panel_attempt(&progress_endpoint, "blocked", Some(progress_code), "Cloudflare/bot challenge"));
    } else if progress_code == 200 {
      match parse_wrapped_data(&progress_text) {
        Ok(progress_data) => {
          if let Some(subscription_remaining) = sub2api_subscription_remaining_usd(&progress_data) {
            remaining_usd += subscription_remaining;
            attempts.push(panel_attempt(&progress_endpoint, "ok", Some(progress_code), "已读取订阅剩余额度"));
          } else {
            attempts.push(panel_attempt(&progress_endpoint, "no_signal", Some(progress_code), "没有可识别订阅剩余额度"));
          }
        }
        Err(error) => attempts.push(panel_attempt(&progress_endpoint, "parse_error", Some(progress_code), &error)),
      }
    } else {
      attempts.push(panel_attempt(&progress_endpoint, "unsupported", Some(progress_code), &short_body_message(&progress_text)));
    }
  }
  ...
  if let Ok((stats_code, stats_headers, stats_text)) = panel_get_text(client, &stats_endpoint.url, headers).await {
    if looks_like_cloudflare_block(stats_code, &stats_headers, &stats_text) {
      attempts.push(panel_attempt(&stats_endpoint, "blocked", Some(stats_code), "Cloudflare/bot challenge"));
    } else if stats_code == 200 {
      match parse_wrapped_data(&stats_text) {
        Ok(stats_data) => {
          used_usd = value_number_any(&stats_data, &["total_actual_cost", "actual_cost", "total_cost"]);
          tokens = value_number_any(&stats_data, &["total_tokens", "token_count", "tokens"]);
          requests = value_number_any(&stats_data, &["total_requests", "request_count", "requests"]);
          attempts.push(panel_attempt(&stats_endpoint, "ok", Some(stats_code), "已读取 dashboard stats"));
        }
        Err(error) => attempts.push(panel_attempt(&stats_endpoint, "parse_error", Some(stats_code), &error)),
      }
    } else {
      attempts.push(panel_attempt(&stats_endpoint, "unsupported", Some(stats_code), &short_body_message(&stats_text)));
    }
  }

  let mut aggregate = RemoteUsageAggregate {
    provider_type: "sub2api".to_string(),
    remaining: Some(NumericField { value: remaining_usd, path: "profile.balance + subscriptions.remaining_usd".to_string() }),
    used: used_usd.map(|value| NumericField { value, path: "stats.total_actual_cost".to_string() }),
    cost: used_usd.map(|value| NumericField { value, path: "stats.total_actual_cost".to_string() }),
    tokens: tokens.map(|value| NumericField { value, path: "stats.total_tokens".to_string() }),
    requests: requests.map(|value| NumericField { value, path: "stats.total_requests".to_string() }),
    balance_unit: "USD".to_string(),
    usage_unit: "USD".to_string(),
    source_endpoints: vec!["sub2api_user_profile".to_string(), "sub2api_subscriptions_progress".to_string(), "sub2api_dashboard_stats".to_string()],
    ..RemoteUsageAggregate::default()
  };
  if let Some(used) = used_usd {
    aggregate.total = Some(NumericField { value: remaining_usd + used, path: "remaining + used".to_string() });
  }
  let mut result = aggregate_to_json(&mut aggregate, &credential.base_url, &credential.provider_key, days, attempts);
  attach_remote_auth_info(&mut result, credential);
  Ok(result)
...
async fn query_remote_usage_with_credential(
  ...
  match credential.auth_mode.as_str() {
    "newapi_password" | "newapi_access_token" => query_newapi_panel_usage(client, credential, days).await,
...
fn resolve_remote_panel_base_url(body: &Value, object: &Map<String, Value>) -> Result<String, String> {
  let direct = clean_string(object.get("baseUrl"));
  if !direct.is_empty() {
    return clean_base(&direct);
  }
  let secret = get_provider_secret(body)?;
  let base_url = secret.get("baseUrl").and_then(Value::as_str).unwrap_or_default().trim();
...
fn panel_type_for_auth_mode(panel_type: &str, auth_mode: &str) -> String {
  ...
  if auth_mode.starts_with("sub2api") { "sub2api".to_string() } else { "newapi".to_string() }
}
...
pub(crate) fn get_provider_remote_usage_credential(query: &Value) -> Result<Value, String> {
  ...
  let tool = if tool.is_empty() { "codex".to_string() } else { tool.to_lowercase() };
...
pub(crate) fn save_provider_remote_usage_credential(body: &Value) -> Result<Value, String> {
  ...
  let tool = if tool_raw.is_empty() { "codex".to_string() } else { tool_raw.to_lowercase() };
  let base_url = resolve_remote_panel_base_url(body, &object)?;
  let panel_type_input = clean_string(object.get("panelType"));
  let auth_mode = normalize_remote_auth_mode(&panel_type_input, &clean_string(object.get("authMode")));
  ...
  let key = remote_panel_credential_key(&tool, &provider_key);
  let records = store.get_mut("records").and_then(Value::as_object_mut).ok_or_else(|| "credential store is invalid".to_string())?;
  records.insert(key, json!({
  ...
  let saved = read_remote_panel_credential(&tool, &provider_key)?.ok_or_else(|| "credential save failed".to_string())?;
...
pub(crate) async fn query_saved_provider_remote_usage(body: &Value) -> Result<Value, String> {
  let object = parse_json_object(body);
  let mode = object.get("mode").and_then(Value::as_str).unwrap_or_default().trim().to_lowercase();
  if mode == "oauth" {
    return Ok(json!({
      "status": "unsupported",
      "supported": false,
  ...
  let provider_key = object.get("providerKey").and_then(Value::as_str).unwrap_or_default().trim().to_string();
  if provider_key.is_empty() {
    return Err("providerKey is required".to_string());
  }
  let tool = object.get("tool").and_then(Value::as_str).unwrap_or("codex").trim().to_lowercase();
  let days = clamp_u64(object.get("days").and_then(Value::as_u64).unwrap_or(30), 1, 366);
  let timeout_ms = clamp_u64(object.get("timeoutMs").and_then(Value::as_u64).unwrap_or(9000), 3000, 20000);
  ...
  if let Some(mut credential) = read_remote_panel_credential(if tool.is_empty() { "codex" } else { &tool }, &provider_key)? {
  ...
  let secret = get_provider_secret(body)?;
  let base_url = secret.get("baseUrl").and_then(Value::as_str).unwrap_or_default().trim();
  let api_key = secret.get("apiKey").and_then(Value::as_str).unwrap_or_default().trim();
  if base_url.is_empty() {
    return Err(format!("Provider {} 未配置 Base URL", provider_key));
  }

  if api_key.is_empty() {
    return Err(format!("Provider {} 未找到 API Key 或远程面板认证", provider_key));
  }
  ...
  for endpoint in endpoints {
    let response = match client.get(&endpoint.url).headers(headers.clone()).send().await {
    ...
    if looks_like_cloudflare_block(status_code, &headers_snapshot, &text) {
      attempts.push(attempt_json(&endpoint, "blocked", Some(status_code), "Cloudflare/bot challenge"));
      return Ok(json!({
        "status": "blocked",
        "supported": false,
    ...
    if status_code == 401 || status_code == 403 {
      saw_auth_error = true;
      attempts.push(attempt_json(&endpoint, "auth_error", Some(status_code), "API Key 无权访问该面板查询接口"));
      continue;
    }
    if status_code == 404 || status_code == 405 {
      attempts.push(attempt_json(&endpoint, "unsupported", Some(status_code), "接口不存在"));
      continue;
    }
    if !status.is_success() {
      attempts.push(attempt_json(&endpoint, "http_error", Some(status_code), &short_body_message(&text)));
      continue;
    }

    let payload = serde_json::from_str::<Value>(&text).unwrap_or_else(|_| json!({ "_text": short_body_message(&text) }));
    ...
    if had_signal {
      attempts.push(attempt_json(&endpoint, "ok", Some(status_code), "已读取余额/用量字段"));
      merge_metrics(&mut aggregate, extracted, endpoint.id);
    } else {
      saw_success_no_signal = true;
      attempts.push(attempt_json(&endpoint, "no_signal", Some(status_code), "响应成功，但没有可识别的余额/用量字段"));
    }
  }
  ...
  if has_signal {
    return Ok(aggregate_to_json(&mut aggregate, &base_url, &provider_key, days, attempts));
  }

  let status = if saw_auth_error && !saw_success_no_signal { "auth_error" } else { "unsupported" };
