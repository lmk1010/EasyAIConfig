...
fn read_state() -> Result<Value, String> {
    ...
    let parsed: Value =
        serde_json::from_str(&text).unwrap_or_else(|_| json!({ "version": 1, "bindings": {} }));
    Ok(parsed)
}
...
fn deepest_match<'a>(bindings: &'a Map<String, Value>, cwd: &str) -> Option<(String, &'a Value)> {
    if let Some(entry) = bindings.get(cwd) {
        return Some((cwd.to_string(), entry));
    }
...
pub(crate) fn set_project_binding(body: &Value) -> Result<Value, String> {
    ...
    let entry = bindings
        .entry(normalized_cwd.clone())
        .or_insert(json!({}))
        .as_object_mut()
        .ok_or("entry is not an object".to_string())?;
    entry.insert(
        normalized_tool.clone(),
        json!({
...
pub(crate) fn list_project_bindings(_query: &Value) -> Result<Value, String> {
    ...
    let items: Vec<Value> = bindings
        .iter()
        .map(|(cwd, tools)| json!({ "cwd": cwd, "tools": tools }))
        .collect();
