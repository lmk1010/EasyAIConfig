...
fn build_default_menu(app: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
  let title = MenuItemBuilder::with_id("__title", "EasyAIConfig").enabled(false).build(app)?;
  let placeholder = MenuItemBuilder::with_id("__sep", "—— 暂未加载 provider ——").enabled(false).build(app)?;
...
pub(crate) fn refresh_menu(app: &AppHandle, body: &Value) -> Result<Value, String> {
  ...
  let providers: Vec<TrayProvider> = serde_json::from_value(
    object.get("providers").cloned().unwrap_or(Value::Array(vec![])),
  )
  ...
  let tray = match app.tray_by_id("ea-config-tray") {
    ...
    None => {
      ...
      app.tray_by_id("ea-config-tray").ok_or_else(|| "tray missing".to_string())?
    }
  };
  ...
  let active_label = providers.iter().find(|p| p.active).map(|p| {
    format!(
      "EasyAIConfig — {} · {}",
      tool_display(&p.tool),
      if p.label.is_empty() { p.key.as_str() } else { p.label.as_str() }
    )
  });
...
fn build_menu_for(app: &AppHandle, providers: &[TrayProvider]) -> tauri::Result<Menu<tauri::Wry>> {
  ...
  let banner_text = providers
    .iter()
    .find(|p| p.active)
    .map(|p| format!("当前: {} · {}", tool_display(&p.tool), if p.label.is_empty() { p.key.as_str() } else { p.label.as_str() }))
    .unwrap_or_else(|| "EasyAIConfig".to_string());
  mb = mb.item(&MenuItemBuilder::with_id("__banner", banner_text).enabled(false).build(app)?);
  mb = mb.separator();

  // 每个 tool 一个 submenu
  let tool_order = ["codex", "claudecode", "opencode", "openclaw"];
  for tool_id in tool_order {
    let Some(items) = by_tool.get(tool_id) else { continue; };
    if items.is_empty() {
      continue;
    }
    ...
    for p in items {
      ...
      let label = if p.label.is_empty() { p.key.clone() } else { p.label.clone() };
