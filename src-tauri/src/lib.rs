...
pub(crate) const GEMINI_CLI_PACKAGE: &str = "@google/gemini-cli";
pub(crate) const QWEN_CODE_PACKAGE: &str = "@qwen-code/qwen-code";
pub(crate) const CODEBUDDY_CODE_PACKAGE: &str = "@tencent-ai/codebuddy-code";
...
fn emit_asset_deep_link_opened(app: &tauri::AppHandle, urls: Vec<String>, source: &str) {
    if urls.is_empty() {
        return;
    }
    let _ = app.emit(
        "asset-deep-link-opened",
        serde_json::json!({
            "urls": urls,
            "source": source,
        }),
        ...
        .setup(|app| {
            ...
            app.deep_link().on_open_url(move |event| {
                let urls = event
                    ...
                    .collect::<Vec<_>>();
                emit_asset_deep_link_opened(&app_handle, urls, "tauri-deep-link-event");
            });
            if let Ok(Some(urls)) = app.deep_link().get_current() {
                emit_asset_deep_link_opened(
                    &app.handle(),
                    urls.into_iter().map(|url| url.to_string()).collect(),
                    "tauri-deep-link-startup",
                );
            }
            #[cfg(any(windows, target_os = "linux"))]
            if let Err(err) = app.deep_link().register_all() {
                log::warn!("deep link registration failed: {err}");
            }
