...
fn read_profiles_index() -> Result<Value, String> {
    ...
    let parsed: Value =
        serde_json::from_str(&text).map_err(|e| format!("profiles.json 解析失败: {}", e))?;
    Ok(parsed)
}
...
pub(crate) fn migrate_auth_json_env_to_codex_env(
    codex_home: &Path,
    auth_raw: &str,
...
pub(crate) fn list_oauth_profiles(_query: &Value) -> Result<Value, String> {
    ...
    let live_meta = oauth_meta_for_codex_home(&current_codex_home).unwrap_or_else(
        |_| json!({ "hasTokens": false, "accountId": "", "plan": "", "email": "", "sub": "" }),
    );
...
pub(crate) fn save_current_oauth_profile(body: &Value) -> Result<Value, String> {
    ...
    {
        return Err(
            "当前 auth.json 没有 OAuth tokens（只有 API Key），请先运行 codex login".to_string(),
        );
    }
        ...
        for p in arr.iter_mut() {
            if p.get("id").and_then(Value::as_str).unwrap_or("") == id {
                if let Some(obj) = p.as_object_mut() {
                    ...
                    obj.insert(
                        "codexHome".to_string(),
                        json!(target_codex_home.to_string_lossy().to_string()),
                    );
