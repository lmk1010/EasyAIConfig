...
use crate::agent_session::{
    agent_close, agent_create, agent_interrupt, agent_list, agent_permission, agent_prompt,
    agent_read, agent_set_mode, agent_set_model, agent_transcript,
};
...
pub(crate) async fn dispatch(
    ...
    match (path, method) {
        ...
        // ─── Agent Timeline（结构化会话，非 PTY）──────────────────
        ("/api/agent/create", "POST") => agent_create(body),
        ("/api/agent/list", "GET") => agent_list(query),
        ("/api/agent/read", "GET") => agent_read(query),
        ("/api/agent/prompt", "POST") => agent_prompt(body),
        ("/api/agent/permission", "POST") => agent_permission(body),
        ("/api/agent/set-mode", "POST") => agent_set_mode(body),
        ("/api/agent/set-model", "POST") => agent_set_model(body),
        ("/api/agent/interrupt", "POST") => agent_interrupt(body),
        ("/api/agent/close", "POST") => agent_close(body),
        ("/api/agent/transcript", "GET") => agent_transcript(query),
