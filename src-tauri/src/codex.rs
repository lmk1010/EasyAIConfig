...
const OPENCLAW_INSTALL_SCRIPT_WIN: &str =
    "$env:OPENCLAW_NO_ONBOARD='1'; iwr -useb https://openclaw.ai/install.ps1 | iex";
const OPENCLAW_NPM_REGISTRY_CN: &str = "https://registry.npmmirror.com";
const CODEX_APP_MAC_DOWNLOAD_URL: &str =
...
static CODEX_APP_INSTALL_TASKS: OnceLock<Mutex<BTreeMap<String, CodexAppInstallTask>>> =
    OnceLock::new();
static OPENCLAW_INSTALL_TASK_SEQ: AtomicU64 = AtomicU64::new(1);
static OPENCLAW_INSTALL_TASKS: OnceLock<Mutex<BTreeMap<String, OpenClawInstallTask>>> =
    OnceLock::new();
...
fn windows_mingit_root() -> Option<PathBuf> {
    app_home()
        .ok()
        .map(|path| path.join("tools").join("mingit"))
}
...
#[cfg(target_os = "windows")]
fn launch_windows_background_command(
    cwd: &Path,
    command_text: &str,
    tool_label: &str,
...
fn opencode_install_steps(action: &str) -> Vec<OpenCodeInstallStep> {
    ...
    specs
        ...
        .enumerate()
        .map(
            |(index, (key, title, description, hint, progress))| OpenCodeInstallStep {
                key: key.to_string(),
...
fn create_opencode_install_task(action: &str, requested_method: &str) -> OpenCodeInstallTask {
    ...
    OpenCodeInstallTask {
        ...
        summary: steps
            ...
            .unwrap_or_else(|| "正在准备任务…".to_string()),
        hint: steps
            .first()
            .map(|step| step.hint.clone())
            .unwrap_or_else(|| "请稍候。".to_string()),
        detail: if action == "uninstall" {
            "正在读取当前安装状态…".to_string()
        } else {
            "正在初始化安装任务…".to_string()
        },
        steps,
...
fn trim_opencode_install_tasks(tasks: &mut BTreeMap<String, OpenCodeInstallTask>) {
    while tasks.len() > OPENCODE_INSTALL_TASK_KEEP {
        let removable = tasks
            .iter()
            .find(|(_, task)| task.status != "running" && task.status != "cancelling")
...
fn with_opencode_install_task<R>(
    task_id: &str,
    mut update: impl FnMut(&mut OpenCodeInstallTask) -> R,
) -> Option<R> {
...
fn set_opencode_install_step(
    task: &mut OpenCodeInstallTask,
    step_index: usize,
            ...
            if cfg!(target_os = "windows") {
                (
                    ...
                    vec![
                        ...
                        "Bypass".to_string(),
                        "-Command".to_string(),
                        OPENCODE_INSTALL_SCRIPT_UNIX.to_string(),
                    ],
                    OPENCODE_INSTALL_SCRIPT_UNIX.to_string(),
                )
            } else {
                (
                    "sh".to_string(),
                    vec!["-lc".to_string(), OPENCODE_INSTALL_SCRIPT_UNIX.to_string()],
                    OPENCODE_INSTALL_SCRIPT_UNIX.to_string(),
                )
            }
        }
...
fn fail_opencode_install_task(task_id: &str, message: String) {
    let _ = with_opencode_install_task(task_id, |task| {
        ...
        task.summary = if task.action == "uninstall" {
            "OpenCode 卸载失败".to_string()
        } else {
            "OpenCode 安装失败".to_string()
        };
...
fn complete_opencode_install_task(task_id: &str, version: Option<String>) {
    let _ = with_opencode_install_task(task_id, |task| {
        ...
        task.progress = 100;
        task.summary = if task.action == "update" {
            "OpenCode 已更新完成".to_string()
        } else if task.action == "reinstall" {
            "OpenCode 已重装完成".to_string()
        } else if task.action == "uninstall" {
            "OpenCode 已卸载完成".to_string()
        } else {
            "OpenCode 已安装完成".to_string()
        };
        task.hint = if task.action == "uninstall" {
            "如需恢复，重新点击安装即可。".to_string()
        } else {
            "下一步可以直接启动 OpenCode，或先去配置 Provider / 模型。".to_string()
        };
        task.detail = if task.action == "uninstall" {
            "已确认 opencode 命令不可用。".to_string()
        } else {
            version
                .clone()
                .map(|v| format!("已检测到版本：{v}"))
                .unwrap_or_else(|| "已检测到 opencode 命令。".to_string())
        };
        task.child_pid = None;
...
fn spawn_opencode_install_task_runner(task_id: String) {
    thread::spawn(move || {
        ...
        let (program, args, display_command) =
            build_opencode_task_command(&action, &effective_method);
        ...
        let _ = with_opencode_install_task(&task_id, |task| {
            ...
            if action == "uninstall" {
                ...
                set_opencode_install_step(
                    task,
                    1,
                    Some(format!("已确认最终方式：{}", task.method)),
                );
            }
        });
        ...
        if action != "uninstall" {
            if let Some(reachable) = google_reachable {
                ...
                push_opencode_install_log(
                    &task_id,
                    "stdout",
                    "本次按你的指定方式执行，未触发 Google 连通性检测。",
                );
            }
            push_opencode_install_log(
                &task_id,
                "stdout",
                &format!("最终安装方式：{}", effective_method),
            );
        } else {
            push_opencode_install_log(
                &task_id,
                "stdout",
                &format!("最终卸载方式：{}", effective_method),
            );
        }
        push_opencode_install_log(
            &task_id,
            "stdout",
            &format!("执行命令：{}", display_command),
        );
            ...
            match (node_output, npm_output) {
                (Ok(node), Ok(npm)) if node.status.success() && npm.status.success() => {
                    ...
                    push_opencode_install_log(
                        &task_id,
                        "stdout",
                        &format!("Node.js {node_version} / npm {npm_version}"),
                    );
                }
                _ => {
                    fail_opencode_install_task(
                        &task_id,
                        "未检测到 Node.js 或 npm，请先安装 Node.js 18+。".to_string(),
                    );
                    return;
                }
            }
        ...
        if effective_method == "script"
            && !cfg!(target_os = "windows")
            && command_exists("curl").is_none()
        {
            fail_opencode_install_task(
                &task_id,
                "未检测到 `curl`，无法执行官方脚本安装。请先安装 curl，或改用 npm 安装。"
                    .to_string(),
            );
            return;
        }
        ...
        if get_opencode_install_task_snapshot(&task_id)
            .map(|task| task.cancel_requested || task.status == "cancelled")
            .unwrap_or(false)
        {
            return;
        }
        ...
        if action == "uninstall" {
                ...
                .unwrap_or(false)
            {
                fail_opencode_install_task(
                    &task_id,
                    "卸载命令已执行完成，但系统里仍检测到 `opencode` 命令。".to_string(),
                );
                return;
            }
        ...
        let _ = with_opencode_install_task(&task_id, |task| {
            set_opencode_install_step(
                task,
                3,
                Some("安装命令执行完成，正在验证 opencode 命令…".to_string()),
            );
        });
            ...
            .unwrap_or(false)
        {
            fail_opencode_install_task(
                &task_id,
                "安装命令已执行完成，但系统里仍未找到 `opencode` 命令。".to_string(),
            );
            return;
        }

        complete_opencode_install_task(
            &task_id,
            binary
                .get("version")
                .and_then(Value::as_str)
...
fn openclaw_install_steps(method: &str) -> Vec<OpenClawInstallStep> {
    let specs = if method == "script" {
        vec![
            (
                ...
                "检查运行环境",
                "确认脚本安装所需命令可用",
                "这一步在确认系统具备安装条件，你不用操作。",
                8_u64,
            ),
            (
                "download",
                "下载官方安装器",
                "从 OpenClaw 官方地址拉取安装脚本",
                "如果网络慢，这一步可能停留几十秒，属于正常现象。",
                24_u64,
            ),
            (
                "install",
                "执行安装脚本",
                "安装器正在写入程序和命令入口",
                "看到日志滚动代表仍在工作，请不要关闭窗口。",
                62_u64,
            ),
            (
                "verify",
                "验证命令是否可用",
                "检查 `openclaw` 是否已能直接运行",
                "已经接近完成，正在做最后确认。",
                88_u64,
            ),
            (
        ...
        vec![
            (
                "preflight",
                "准备国内安装环境",
                "检查 Node.js、npm，并优先启用应用内 Git",
                "这一步会尽量自动补齐缺失依赖，你不用手动处理。",
                8_u64,
            ),
            (
                "download",
                "切换国内 npm 源",
                "使用 npmmirror 获取 OpenClaw 安装包和依赖",
                "国内网络下通常会更稳、更快。",
                26_u64,
            ),
            (
    ...
    specs
        ...
        .enumerate()
        .map(
            |(index, (key, title, description, hint, progress))| OpenClawInstallStep {
                key: key.to_string(),
                title: title.to_string(),
                description: description.to_string(),
                hint: hint.to_string(),
...
fn capture_openclaw_install_snapshot() -> OpenClawInstallSnapshot {
    ...
    let npm_prefix = if cfg!(target_os = "windows") {
        windows_user_npm_prefix()
            .map(|p| p.to_string_lossy().to_string())
            .unwrap_or_default()
    } else {
        npm_global_output(&["prefix", "-g"])
    };
    let npm_root = if cfg!(target_os = "windows") {
        windows_user_npm_prefix()
            .map(|p| p.join("node_modules").to_string_lossy().to_string())
            .unwrap_or_default()
    } else {
        npm_global_output(&["root", "-g"])
    };
    let package_path = if npm_root.is_empty() {
        String::new()
    } else {
        PathBuf::from(&npm_root)
...
fn trim_openclaw_install_tasks(tasks: &mut BTreeMap<String, OpenClawInstallTask>) {
    while tasks.len() > OPENCLAW_INSTALL_TASK_KEEP {
        let removable = tasks
            .iter()
            .find(|(_, task)| !is_openclaw_install_active(task))
            .map(|(task_id, _)| task_id.clone());
        if let Some(task_id) = removable {
            tasks.remove(&task_id);
        } else {
...
fn with_openclaw_install_task<R>(
    task_id: &str,
    mut update: impl FnMut(&mut OpenClawInstallTask) -> R,
) -> Option<R> {
...
fn set_openclaw_install_step(
    task: &mut OpenClawInstallTask,
    step_index: usize,
    detail: Option<String>,
) {
    if task.steps.is_empty() {
...
fn infer_openclaw_install_step(task: &mut OpenClawInstallTask, line: &str) {
    ...
    if task.method == "script" {
        if text.contains("[1/3]")
            || text.contains("preparing environment")
            || text.contains("homebrew")
            || text.contains("node.js")
            || text.contains("active npm")
            || text.contains("active node")
        {
...
fn cleanup_cancelled_openclaw_install(task: &mut OpenClawInstallTask) -> Vec<String> {
    ...
    if !snapshot.had_binary {
        ...
        for target in std::iter::once(snapshot.package_path.clone())
            .chain(snapshot.bin_paths.clone().into_iter())
        {
...
fn cancel_openclaw_install_task_inner(task_id: &str) -> Result<(), String> {
    ...
    let _ = with_openclaw_install_task(task_id, |task| {
        ...
        task.progress = 100;
        task.child_pid = None;
        task.error = if cleanup_errors.is_empty() {
            None
        } else {
        ...
        task.detail = task
            .error
            .clone()
            .unwrap_or_else(|| "未发现需要额外清理的残留。".to_string());
...
fn complete_openclaw_install_task(task_id: &str, version: Option<String>) {
    let _ = with_openclaw_install_task(task_id, |task| {
        ...
        if let Some(last_index) = task.steps.len().checked_sub(1) {
            ...
            for (index, step) in task.steps.iter_mut().enumerate() {
                step.status = if index <= last_index {
                    "done".to_string()
                } else {
                    "pending".to_string()
                };
        ...
        task.detail = version
            .clone()
            .map(|v| format!("已检测到版本：{v}"))
            .unwrap_or_else(|| "已检测到 openclaw 命令。".to_string());
        task.child_pid = None;
        task.version = version.clone();
        task.next_actions = vec![
...
fn spawn_openclaw_install_task_runner(task_id: String) {
    thread::spawn(move || {
        ...
        if get_openclaw_install_task_snapshot(&task_id)
            .map(|task| is_openclaw_install_cancelled(&task))
            .unwrap_or(false)
        {
            return;
        }
        ...
        if current_method == "script" {
            if !cfg!(target_os = "windows") && command_exists("curl").is_none() {
                fail_openclaw_install_task(
                    &task_id,
                    "未检测到 `curl`，无法执行脚本安装。请先安装 curl，或改用 npm 安装。"
                        .to_string(),
                );
                return;
            }
            if command_exists("git").is_none() {
                ...
                if !auto_installed {
                    ...
                    push_openclaw_install_log(
                        &task_id,
                        "stdout",
                        "Git 自动安装失败，尝试自动切换为一键安装方式…",
                    );
                    ...
                    if node_ok && npm_ok {
                        push_openclaw_install_log(
                            ...
                            "stdout",
                            &format!(
                                "已自动切换为一键安装方式，将使用国内 npm 源：{}",
                                OPENCLAW_NPM_REGISTRY_CN
                            ),
                        );
        ...
        if current_method == "domestic" {
            push_openclaw_install_log(
                &task_id,
                "stdout",
                &format!("已启用国内 npm 源：{}", OPENCLAW_NPM_REGISTRY_CN),
            );
            if cfg!(target_os = "windows") && command_exists("git").is_none() {
                push_openclaw_install_log(
                    &task_id,
                    "stdout",
                    "一键安装模式正在检查 Git，可选依赖将优先使用应用内 MinGit…",
                );
                if !try_auto_install_git(&task_id) {
                    push_openclaw_install_log(
            &task_id,
            "stderr",
            "应用内 Git 预安装失败，将继续尝试 npm 安装；如依赖需要 Git，日志会继续提示。",
          );
                }
            }
        }
        ...
        if current_method == "npm" || current_method == "domestic" {
            ...
            match (node_output, npm_output) {
                ...
                (Ok(_), Ok(_)) => {
                    fail_openclaw_install_task(
                        &task_id,
                        "未检测到可用的 Node.js / npm，请先修复运行环境后重试。".to_string(),
                    );
                    return;
                }
                _ => {
        ...
        let _ = with_openclaw_install_task(&task_id, |task| {
            set_openclaw_install_step(
                task,
                3,
                Some("安装命令已执行完成，正在验证 openclaw 命令…".to_string()),
            );
        });
            ...
            .unwrap_or(false)
        {
            fail_openclaw_install_task(
                &task_id,
                "安装命令已执行完成，但系统里仍未找到 `openclaw` 命令。".to_string(),
            );
            return;
        }

        complete_openclaw_install_task(
            &task_id,
            binary
                .get("version")
                .and_then(Value::as_str)
...
fn parse_git_windows_release_candidates(listing: &str) -> Vec<String> {
    ...
    releases.sort_by(|left, right| {
        ...
        let (left_core, left_rev) = left_trimmed
            .rsplit_once(".windows.")
            .unwrap_or((left_trimmed, "0"));
        let (right_core, right_rev) = right_trimmed
            .rsplit_once(".windows.")
            .unwrap_or((right_trimmed, "0"));
        compare_versions(left_core, right_core).then_with(|| {
            left_rev
                .parse::<u32>()
                .unwrap_or(0)
                .cmp(&right_rev.parse::<u32>().unwrap_or(0))
        })
    });
    releases.dedup();
    releases
...
fn resolve_mingit_download_urls(task_id: &str) -> Result<Vec<String>, String> {
    ...
    for base in bases {
        push_openclaw_install_log(
            task_id,
            "stdout",
            &format!("正在获取华为云 Git 镜像目录：{}", base),
        );
        let base_listing = match client
            .get(base)
            .send()
            .and_then(|response| response.error_for_status())
        {
            ...
            Err(error) => {
                push_openclaw_install_log(
                    task_id,
                    "stderr",
                    &format!("华为云目录访问失败：{}", error),
                );
                continue;
            }
        };

        let latest_release = match parse_git_windows_release_candidates(&base_listing)
            .into_iter()
            .next_back()
        {
        ...
        push_openclaw_install_log(
            task_id,
            "stdout",
            &format!("正在获取华为云版本目录：{}", release_url),
        );
        let release_listing = match client
            .get(&release_url)
            .send()
            .and_then(|response| response.error_for_status())
        {
        ...
        assets.sort_by(|left, right| {
            let lv = left
                .trim_start_matches("MinGit-")
                .trim_end_matches("-64-bit.zip");
            let rv = right
                .trim_start_matches("MinGit-")
                .trim_end_matches("-64-bit.zip");
...
fn install_mingit_from_mirror(task_id: &str) -> bool {
    ...
    for download_url in download_urls {
        push_openclaw_install_log(
            task_id,
            "stdout",
            &format!("正在下载 MinGit：{}", download_url),
        );
...
fn resolve_portable_node_download_url(task_id: &str) -> Result<String, String> {
    ...
    push_openclaw_install_log(
        task_id,
        "stdout",
        &format!("正在获取 Node.js 镜像索引：{}", shasums_url),
    );
    let client = reqwest::blocking::Client::builder()
        .timeout(Duration::from_secs(20))
        .build()
    ...
    Ok(format!(
        "https://cdn.npmmirror.com/binaries/node/latest-v22.x/{}",
        asset
    ))
}
...
fn install_portable_node_from_mirror(task_id: &str) -> bool {
    ...
    if let Err(error) = ensure_dir(&tools_dir) {
        push_openclaw_install_log(
            task_id,
            "stderr",
            &format!("创建 Node.js 工具目录失败：{}", error),
        );
        return false;
    }
    ...
    push_openclaw_install_log(
        task_id,
        "stdout",
        &format!("正在下载便携版 Node.js：{}", download_url),
    );
    if archive_path.exists() {
        let _ = fs::remove_file(&archive_path);
    }
    ...
    if windows_portable_node_dirs().is_empty() {
        push_openclaw_install_log(
            task_id,
            "stderr",
            "便携版 Node.js 解压完成，但未找到 node.exe",
        );
        return false;
    }
    ...
    if node_ok && npm_ok {
        push_openclaw_install_log(
            task_id,
            "stdout",
            "✓ 已自动安装便携版 Node.js / npm，将使用应用内置运行时",
        );
        return true;
    }
    ...
    push_openclaw_install_log(
        task_id,
        "stderr",
        "便携版 Node.js 已下载，但仍未检测到 node / npm",
    );
    false
}
...
fn ensure_node_and_npm_available(task_id: &str) -> bool {
    ...
    if cfg!(target_os = "windows") {
        push_openclaw_install_log(
            task_id,
            "stdout",
            "未检测到可用的 Node.js / npm，正在尝试自动安装便携版 Node.js…",
        );
...
fn try_auto_install_git(task_id: &str) -> bool {
    ...
    if cfg!(target_os = "windows") {
            ...
            Ok(out) => {
                ...
                if out.status.success() {
                    ...
                    push_openclaw_install_log(
                        task_id,
                        "stderr",
                        "winget 已执行但仍未检测到 Git，尝试安装内置 MinGit…",
                    );
                } else {
                    push_openclaw_install_log(
                        task_id,
                        "stderr",
                        "winget 安装 Git 失败，尝试安装内置 MinGit…",
                    );
                }
            }
            Err(e) => {
                ...
                Ok(out) => {
                    ...
                    if out.status.success() {
                        ...
                        push_openclaw_install_log(
                            task_id,
                            "stderr",
                            &format!("brew install git 失败：{}", stderr_text.trim()),
                        );
                    }
                }
                Err(e) => {
        ...
        push_openclaw_install_log(
            task_id,
            "stderr",
            "Linux 上需要手动安装 Git（如 sudo apt install git）",
        );
    }
...
fn macos_terminal_cli_command(profile: &str) -> Option<String> {
    match profile {
        "wezterm" => command_exists("wezterm")
            .or_else(|| macos_bundle_executable(&["WezTerm"], &["wezterm"])),
        "ghostty" => command_exists("ghostty")
            .or_else(|| macos_bundle_executable(&["Ghostty"], &["ghostty"])),
        "alacritty" => command_exists("alacritty")
            .or_else(|| macos_bundle_executable(&["Alacritty"], &["alacritty"])),
        "kitty" => {
            command_exists("kitty").or_else(|| macos_bundle_executable(&["kitty"], &["kitty"]))
        }
        _ => None,
    }
}
...
fn macos_terminal_profile_available(profile: &str) -> bool {
    match profile {
        ...
        "wezterm" | "ghostty" | "alacritty" | "kitty" => {
            macos_terminal_cli_command(profile).is_some()
        }
        "termius" | "terminus" | "tabby" | "warp" | "hyper" => generic_macos_terminal_app(profile)
...
fn resolve_macos_terminal_profile(profile: &str) -> String {
    ...
    let requested = match normalized.as_str() {
        "terminal" | "iterm" | "termius" | "terminus" | "tabby" | "warp" | "hyper" | "wezterm"
        | "ghostty" | "alacritty" | "kitty" => normalized.as_str(),
        _ => "auto",
    };
...
fn launch_macos_app_and_type_command(
    app_name: &str,
    app_label: &str,
    shell_command: &str,
    tool_label: &str,
...
fn launch_macos_terminal_with_profile(
    ...
    let shell_command = format!(
        "cd {} && {}",
        quote_posix_shell_arg(&cwd.to_string_lossy()),
        command_text
    );
    ...
    let app_label = if resolved == "iterm" {
        "iTerm"
    } else {
        "Terminal"
    };
...
fn linux_terminal_args(profile: &str, cwd: &Path, shell_command: &str) -> Vec<String> {
    ...
    match profile {
        ...
        "xfce4-terminal" => vec![
            ...
            cwd_text,
            "-e".to_string(),
            format!("bash -lc {}", quote_posix_shell_arg(shell_command)),
        ],
        "lxterminal" => vec![
            "--working-directory".to_string(),
            cwd_text,
...
fn launch_linux_terminal_with_profile(
    cwd: &Path,
    command_text: &str,
    tool_label: &str,
    terminal_profile: &str,
...
fn collect_codex_provider_env_keys(codex_home: &Path, cwd: Option<&Path>) -> Vec<String> {
    ...
    if let Some(project_dir) = cwd {
        collect_codex_provider_env_keys_from_config(
...
fn clear_codex_oauth_active_provider(
    codex_home: &Path,
    cwd: Option<&Path>,
) -> Result<bool, String> {
...
fn codex_launch_extra_env(codex_home: &Path, cwd: Option<&Path>) -> Vec<(String, String)> {
    ...
    if codex_launch_uses_local_router(codex_home, cwd) {
        env.push((
            "NO_PROXY".to_string(),
            LOCAL_ROUTER_NO_PROXY_VALUE.to_string(),
        ));
        env.push((
            "no_proxy".to_string(),
            LOCAL_ROUTER_NO_PROXY_VALUE.to_string(),
...
fn resolve_windows_binary_invocation_path(binary_path: &str, fallback_binary: &str) -> String {
    ...
    if !normalized.is_empty() {
        ...
        if looks_like_path
            ...
            || has_windows_invocation_extension(&lower)
        {
            return normalized;
        }
...
fn launch_codex_terminal_command(
    ...
    let command_text = if cfg!(target_os = "windows") {
        ...
        with_codex_home_command(
            &build_windows_binary_command(codex_path, &empty_args, "codex"),
            codex_home,
            Some(cwd),
        )
    } else {
...
pub(crate) fn get_codex_release_info() -> Result<Value, String> {
    ...
    let tags =
        serde_json::from_str::<Value>(result.get("stdout").and_then(Value::as_str).unwrap_or("{}"))
    ...
    let latest_stable = tags
        .get("latest")
        .and_then(Value::as_str)
        .map(|text| text.to_string());
    let latest_alpha = tags
        .get("alpha")
        .and_then(Value::as_str)
        .map(|text| text.to_string());
...
fn extract_codex_user_message_preview(event: &Value) -> String {
    ...
    if event_type == "event_msg"
        && payload
            ...
            == Some("user_message")
    {
        return normalize_codex_session_preview(
            payload
...
fn is_same_or_nested_path(left: &str, right: &str) -> bool {
    let Some(left_path) = normalize_path_for_compare(left) else {
        return false;
    };
    let Some(right_path) = normalize_path_for_compare(right) else {
        return false;
    };
...
fn read_codex_session_summary(file_path: &Path, modified_ms: u64) -> Option<Value> {
        ...
        if event_type == "session_meta" {
            if let Some(payload) = event.get("payload").and_then(Value::as_object) {
                let meta_id = payload
                    ...
                    .unwrap_or("")
                    .trim();
                let meta_cwd = payload
                    .get("cwd")
                    .and_then(Value::as_str)
                    .unwrap_or("")
                    .trim();
                let meta_provider = payload
                    .get("model_provider")
                    .and_then(Value::as_str)
                    .unwrap_or("")
                    .trim();
                let meta_model = payload
                    .get("model")
                    .and_then(Value::as_str)
                    .unwrap_or("")
                    .trim();
                ...
                if !meta_cwd.is_empty() {
                    cwd = meta_cwd.to_string();
                }
                if !meta_provider.is_empty() {
                    provider = meta_provider.to_string();
                }
                if !meta_model.is_empty() {
                    model = meta_model.to_string();
                }
            }
            continue;
        }
    ...
    let updated_at = chrono::DateTime::<chrono::Utc>::from_timestamp_millis(
        updated_at_ms.min(i64::MAX as u64) as i64,
    )
...
fn build_codex_session_command(binary_path: &str, args: &[String]) -> String {
    ...
    let binary = if binary_path.trim().is_empty() {
        ...
        quote_posix_shell_arg(binary_path)
    };
...
pub(crate) fn list_codex_sessions(query: &Value) -> Result<Value, String> {
    ...
    let max_items = get_string(&query_object, "limit")
        .parse::<usize>()
        .ok()
        .unwrap_or(20)
        .clamp(1, 100);
    let show_all = matches!(
        get_string(&query_object, "all")
            .trim()
            .to_lowercase()
            .as_str(),
        "1" | "true" | "yes"
    ...
    let mut file_entries = list_jsonl_files(&sessions_root)
        .into_iter()
        .map(|path| CodexUsageSessionFile {
    ...
    for entry in file_entries {
        ...
        let Some(summary) = read_codex_session_summary(&entry.path, entry.modified_ms) else {
            continue;
        };
        let session_cwd = summary.get("cwd").and_then(Value::as_str).unwrap_or("");
        if !show_all
            && !target_cwd.trim().is_empty()
            && !is_same_or_nested_path(session_cwd, &target_cwd)
        {
            continue;
        }
        items.push(summary);
...
pub(crate) fn get_codex_session_detail(query: &Value) -> Result<Value, String> {
    ...
    let file_path =
        resolve_codex_session_path(&get_string(&query_object, "filePath"), &codex_home)?;
    let preview_limit = get_string(&query_object, "limit")
        .parse::<usize>()
        .ok()
        .unwrap_or(120)
        .clamp(20, 500);
...
pub(crate) fn migrate_codex_sessions(body: &Value) -> Result<Value, String> {
    ...
    let dry_run = object
        ...
        .unwrap_or(false);
    let source =
        expand_home_path(&source_input).ok_or_else(|| "sourceCodexHome 不能为空".to_string())?;
    let target =
        expand_home_path(&target_input).ok_or_else(|| "targetCodexHome 不能为空".to_string())?;
    if source == target {
    ...
    if !source_sessions.exists() {
        return Err(format!(
            "源 home 下没有 sessions/ 目录：{}",
            source_sessions.to_string_lossy()
        ));
    }
    ...
    for src_path in files.iter() {
        let rel = match src_path.strip_prefix(&source_sessions) {
            ...
            Err(_) => {
                errors.push(
          json!({ "path": src_path.to_string_lossy().to_string(), "error": "无法计算相对路径" }),
        );
                continue;
            }
        };
        ...
        if let Some(parent) = dst_path.parent() {
            if let Err(error) = ensure_dir(parent) {
                errors.push(
                    json!({ "path": src_path.to_string_lossy().to_string(), "error": error }),
                );
                continue;
            }
        }
...
pub(crate) fn list_codex_session_homes(_query: &Value) -> Result<Value, String> {
    ...
    if let Some(profiles) = profiles_state.get("profiles").and_then(Value::as_array) {
        for profile in profiles {
            ...
            let count = if sessions.exists() {
                list_jsonl_files(&sessions).len()
            } else {
                0
            };
            let label = profile
...
fn collect_storage_usage(path: &Path) -> (bool, bool, u64, u64) {
    ...
    let bytes = files.iter().fold(0u64, |sum, file| {
        sum.saturating_add(fs::metadata(file).map(|meta| meta.len()).unwrap_or(0))
    });
...
pub(crate) fn get_system_storage_state() -> Result<Value, String> {
    ...
    let total_bytes = entries.iter().fold(0u64, |sum, item| {
        sum.saturating_add(item.get("bytes").and_then(Value::as_u64).unwrap_or(0))
    });
    let total_files = entries.iter().fold(0u64, |sum, item| {
        sum.saturating_add(item.get("fileCount").and_then(Value::as_u64).unwrap_or(0))
    });
...
pub(crate) fn cleanup_system_storage(body: &Value) -> Result<Value, String> {
    let object = parse_json_object(body);
    let clear_cache = object
        .get("clearCache")
        .and_then(Value::as_bool)
        .unwrap_or(true);
    let clear_backups = object
        .get("clearBackups")
        .and_then(Value::as_bool)
        .unwrap_or(false);
    ...
    if clear_cache {
        ...
        if cache_dir.exists() {
            match fs::remove_dir_all(&cache_dir) {
                ...
                Err(error) => {
                    failed_paths.push(format!("{}: {}", cache_dir.to_string_lossy(), error))
                }
            }
        }
    }
    ...
    if clear_backups {
        ...
        if backups_dir.exists() {
            match fs::remove_dir_all(&backups_dir) {
                ...
                Err(error) => {
                    failed_paths.push(format!("{}: {}", backups_dir.to_string_lossy(), error))
                }
            }
        }
    }
...
fn claude_usage_cache_key(telemetry_root: &Path, day_count: i64) -> String {
    format!(
        "v{}::{}::{}",
        USAGE_PRICING_VERSION,
        telemetry_root.to_string_lossy(),
        day_count
    )
}

fn read_claude_usage_cache(
    cache_path: &Path,
    telemetry_root: &Path,
    day_count: i64,
    file_count: u64,
...
fn write_claude_usage_cache(
    ...
    let Ok(connection) = open_codex_usage_cache_db(cache_path) else {
        return;
    };
...
fn list_recent_session_files(
    root: &Path,
    window_start_day: chrono::NaiveDate,
) -> Vec<CodexUsageSessionFile> {
...
pub(crate) fn get_codex_usage_metrics(query: &Value) -> Result<Value, String> {
    ...
    let force_refresh = matches!(
        get_string(&query_object, "force").as_str(),
        "1" | "true" | "yes"
    );
    ...
    let latest_mtime_ms = session_files
        .iter()
    ...
    if !force_refresh {
        if let Some(payload) = read_codex_usage_cache(
            &cache_path,
            &sessions_root,
            day_count,
            file_count,
        ...
        for line in reader.lines().map_while(Result::ok) {
            ...
            let provider_stat =
                by_provider
                    ...
                    .or_insert_with(|| CodexUsageProviderStat {
            ...
            let model_stat =
                by_model
                    ...
                    .or_insert_with(|| CodexUsageModelStat {
            ...
            let session_stat =
                by_session
                    ...
                    .or_insert_with(|| CodexUsageSessionStat {
...
mod tests {
    ...
    fn probe_codex_usage_metrics_runtime() {
        ...
        let cached_result =
            get_codex_usage_metrics(&json!({ "days": "30" })).expect("cached metrics");
      ...
      usage.get("dailyModelTokens").and_then(Value::as_array).map(|a| a.len()).unwrap_or(0),
        ...
        if let Some(models) = usage.get("models").and_then(Value::as_array) {
            for m in models {
                ...
                eprintln!(
                    "  model={} total={} input={} output={} source={}",
                    name, total, input, output, source
                );
            }
        }
        assert!(
...
pub(crate) fn check_setup_environment(query: &Value) -> Result<Value, String> {
    ...
    let codex_installed = codex_binary
        .get("installed")
        .and_then(Value::as_bool)
        .unwrap_or(false);
    ...
    let has_login = login
        ...
        .unwrap_or(false);
    let needs_setup =
        !codex_installed || (!config_exists && !has_login) || (!has_providers && !has_login);
...
use crate::{
    app_home, backups_root, claude_code_home, compare_versions, default_codex_home,
    effective_claude_code_home, ensure_dir, expand_home_path, extract_version, home_dir,
    npm_command, openclaw_home, opencode_config_home, opencode_data_home, parse_json_object,
    parse_toml_config, read_text, stringify_toml_config, write_text, CLAUDE_CODE_PACKAGE,
    CODEBUDDY_CODE_PACKAGE, GEMINI_CLI_PACKAGE, OPENAI_CODEX_PACKAGE, OPENCLAW_PACKAGE,
    OPENCODE_PACKAGE, QWEN_CODE_PACKAGE,
};
...
fn find_tool_binary_with_options(binary_name: &str, passive: bool) -> Value {
    ...
    if binary_name.eq_ignore_ascii_case("opencode") {
        if let Ok(home) = home_dir() {
            let unix_candidates = [
                std::env::var("OPENCODE_INSTALL_DIR")
                    .ok()
                    .filter(|value| !value.trim().is_empty())
                    .map(|value| PathBuf::from(value).join("opencode")),
                std::env::var("XDG_BIN_DIR")
                    .ok()
                    .filter(|value| !value.trim().is_empty())
                    .map(|value| PathBuf::from(value).join("opencode")),
...
pub(crate) fn list_tools() -> Result<Value, String> {
    ...
    let claude_desktop_binary = find_tool_binary_with_options("Claude", passive);
    let gemini_binary = find_tool_binary_with_options("gemini", passive);
    let qwen_binary = find_tool_binary_with_options("qwen", passive);
    let codebuddy_binary = find_tool_binary_with_options("codebuddy", passive);
    ...
    let hermes_binary = find_tool_binary_with_options("hermes", passive);
    ...
    Ok(json!([
      ...
      },
      {
        ...
        "npmPackage": "",
        "binary": claude_desktop_binary,
      },
      {
        "id": "gemini",
        "name": "Gemini CLI",
        "description": "Google Gemini 命令行编程助手",
        "supported": true,
        "supportStage": "active",
        "configFormat": "json",
        "installMethod": "npm",
      ...
      },
      {
        "id": "qwen-code",
        "name": "Qwen Code CLI",
        "description": "阿里 Qwen Code 命令行工具，支持安装、更新和历史版本",
        "supported": true,
        "supportStage": "active",
      ...
      },
      {
        ...
        "description": "腾讯 CodeBuddy CLI，支持安装、更新和历史版本",
        "supported": true,
        "supportStage": "active",
        "configFormat": "unknown",
        "installMethod": "npm",
        "npmPackage": CODEBUDDY_CODE_PACKAGE,
        "binary": codebuddy_binary,
      },
      {
        "id": "opencode",
        "name": "OpenCode",
      ...
      },
      {
        "id": "hermes",
        "name": "Hermes Agent",
        "description": "Hermes Agent 工作区与会话能力",
        "supported": true,
        "supportStage": "active",
        "configFormat": "yaml",
        "installMethod": "manual",
...
fn npm_package_latest_url(registry: &str, package_name: &str) -> String {
    format!(
        "{}/latest",
        npm_package_metadata_url(registry, package_name)
    )
}
...
fn fetch_github_release_notes(repository_url: &str) -> BTreeMap<String, Value> {
    ...
    for release in releases {
        ...
        for key in release_version_candidates(&release) {
            by_version
                .entry(key)
                .or_insert_with(|| release_info.clone());
        }
    }
    by_version
}
...
fn merge_latest_npm_metadata(package_name: &str, latest: &Value, metadata: Value) -> Value {
    ...
    if let Some(dist_tags) = dist_tags_value.as_object_mut() {
        ...
        map.insert(
            "dist-tags".to_string(),
            json!({ "latest": latest_version.clone() }),
        );
    }

    let versions_value = map
        ...
        .is_empty()
    {
        map.insert(
            ...
            latest
                .get("name")
                .cloned()
                .unwrap_or_else(|| json!(package_name)),
        );
    }
    if map
        .get("description")
    ...
    if !map.contains_key("repository") || map.get("repository").map(Value::is_null).unwrap_or(true)
    {
        map.insert(
            ...
            latest.get("repository").cloned().unwrap_or(Value::Null),
        );
    }
    if map
        .get("homepage")
        ...
        .is_empty()
    {
        map.insert(
            "homepage".to_string(),
            latest.get("homepage").cloned().unwrap_or_else(|| json!("")),
        );
    }
    if !map.contains_key("bugs") || map.get("bugs").map(Value::is_null).unwrap_or(true) {
        map.insert(
            "bugs".to_string(),
            latest.get("bugs").cloned().unwrap_or(Value::Null),
        );
    }
...
fn recent_npm_versions(
    ...
    names.sort_by(|left, right| {
        let left_time = time
            .and_then(|map| map.get(left))
            .and_then(Value::as_str)
            .unwrap_or("");
        let right_time = time
            .and_then(|map| map.get(right))
            .and_then(Value::as_str)
            .unwrap_or("");
...
fn fetch_tool_update_info(
    ...
    for (source_id, source_label, registry) in sources {
        ...
        thread::spawn(move || {
            let _ = tx.send((
                ...
                source_label,
                registry,
                fetch_npm_metadata(registry, &package_name),
            ));
        });
    }
    drop(tx);
    ...
    while attempts.len() < source_count {
        match rx.recv_timeout(Duration::from_millis(5500)) {
            ...
            Ok((source_id, source_label, _, Err(error))) => {
                attempts.push(
                    json!({ "id": source_id, "label": source_label, "ok": false, "error": error }),
                );
            }
            Err(_) => {
                attempts.push(
                    json!({ "id": "unknown", "label": "版本源", "ok": false, "error": "任务超时" }),
                );
                break;
            }
        }
    ...
    let message = attempts
        ...
        .filter_map(|item| {
            ...
            if label.is_empty() && error.is_empty() {
                None
            } else {
...
pub(crate) fn get_tool_updates_info() -> Result<Value, String> {
    let specs = [
        ...
        (
            "codebuddy-code",
            "CodeBuddy Code CLI",
            CODEBUDDY_CODE_PACKAGE,
            "https://cnb.cool/codebuddy/codebuddy-code",
        ),
        (
            "opencode",
            "OpenCode",
    ...
    let handles = specs
        ...
        .map(|(tool_id, name, package_name, repository_url_hint)| {
            thread::spawn(move || {
                (
                    tool_id.to_string(),
                    fetch_tool_update_info(tool_id, name, package_name, repository_url_hint),
                )
            })
        })
        .collect::<Vec<_>>();
...
fn create_codex_app_install_task(reinstall: bool) -> CodexAppInstallTask {
    ...
    CodexAppInstallTask {
        ...
        step_index: 0,
        summary: if reinstall {
            "正在更新 Codex App…".to_string()
        } else {
            "正在安装 Codex App…".to_string()
        },
        hint: if cfg!(target_os = "windows") {
            "会打开 Microsoft Store，请在商店里确认安装或更新。".to_string()
        } else {
            "会自动下载、挂载并复制到 Applications。".to_string()
        },
...
fn with_codex_app_install_task<R>(
    task_id: &str,
    mut update: impl FnMut(&mut CodexAppInstallTask) -> R,
) -> Option<R> {
...
fn set_codex_app_install_step(
    task: &mut CodexAppInstallTask,
    step_index: usize,
    detail: Option<String>,
) {
    if task.steps.is_empty() {
...
fn finalize_codex_app_cancelled(task_id: &str, reason: &str) {
    let _ = with_codex_app_install_task(task_id, |task| {
        ...
        task.detail = if reason.trim().is_empty() {
            "已停止当前安装任务。".to_string()
        } else {
            reason.to_string()
        };
...
fn is_codex_app_install_cancelled(task_id: &str) -> bool {
    get_codex_app_install_task_snapshot(task_id)
        .map(|task| {
            task.cancel_requested || task.status == "cancelled" || task.status == "cancelling"
        })
        .unwrap_or(true)
}
...
fn download_codex_app_dmg(task_id: &str, destination: &Path) -> Result<(), String> {
    ...
    let mut next_log_bytes = if total_bytes > 0 {
        (total_bytes / 10).max(1)
    } else {
        5 * 1024 * 1024
    };
    ...
    loop {
        ...
        if downloaded_bytes >= next_log_bytes {
            ...
            let total_mb = if total_bytes > 0 {
                format!("{:.1}", total_bytes as f64 / 1024.0 / 1024.0)
            } else {
                "?".to_string()
            };
            push_codex_app_install_log(
                task_id,
                "stdout",
                &format!("已下载 {:.1} MB / {} MB", downloaded_mb, total_mb),
            );
            let _ = with_codex_app_install_task(task_id, |task| {
                ...
                task.progress = task
                    .progress
                    .max((46.0 + ratio.min(1.0) * 32.0).round() as u64)
                    .min(80);
                task.download_path = Some(destination.to_path_buf());
                touch_codex_app_install_task(task);
            });
            next_log_bytes += if total_bytes > 0 {
                (total_bytes / 10).max(1)
            } else {
                5 * 1024 * 1024
            };
        ...
        for app_dir in targets {
            ...
            if copy.status.success() {
                ...
                let _ = create_command("xattr")
                    .args(["-dr", "com.apple.quarantine", &target_text])
                    .output();
...
fn spawn_codex_app_install_task_runner(task_id: String) {
    thread::spawn(move || {
        ...
        if !supported {
            fail_codex_app_install_task(
                &task_id,
                "当前系统暂不支持 Codex App 自动安装".to_string(),
            );
            return;
        }
        ...
        if cfg!(target_os = "windows") {
            let _ = with_codex_app_install_task(&task_id, |task| {
                set_codex_app_install_step(
                    task,
                    1,
                    Some("正在交给 Microsoft Store 处理…".to_string()),
                );
            });
            ...
            complete_codex_app_install_task(
                &task_id,
                "Codex App 商店入口已打开",
                "在 Microsoft Store 里确认即可完成安装或更新。",
            );
            return;
        }
        ...
        match install_codex_app_dmg(&task_id, &installer_path) {
            Ok(_installed_path) => {
                    ...
                    .unwrap_or(false)
                {
                    fail_codex_app_install_task(
                        &task_id,
                        "安装命令完成，但还未检测到 Codex App".to_string(),
                    );
                    return;
                }
                let version = state
                    ...
                    .unwrap_or("")
                    .trim();
                let hint = if version.is_empty() {
                    "Codex App 已准备好。".to_string()
                } else {
                    format!("当前版本 {}", version)
                };
                let summary = with_codex_app_install_task(&task_id, |task| {
                    if task.action == "reinstall" {
                        "Codex App 更新完成".to_string()
                    } else {
                        "Codex App 安装完成".to_string()
                    }
                })
...
fn read_plist_string_value(text: &str, key: &str) -> String {
    ...
    let Some(start_pos) = tail.find("<string>") else {
        return String::new();
    };
    let value_tail = &tail[start_pos + "<string>".len()..];
    let Some(end_pos) = value_tail.find("</string>") else {
        return String::new();
    };
...
fn read_mac_app_bundle_version(app_path: &Path) -> String {
    ...
    read_plist_string_value(&text, "CFBundleVersion")
        .trim()
        .to_string()
}
...
pub(crate) fn start_codex_app_install_task(body: &Value) -> Result<Value, String> {
    ...
    let reinstall = matches!(
        get_string(&obj, "reinstall").trim().to_lowercase().as_str(),
        "1" | "true" | "yes"
    );
...
fn read_shell_anthropic_env() -> std::collections::HashMap<String, String> {
    ...
    fn extract(output: &str, prefix: &str, suffix: &str) -> String {
        output
            ...
            .and_then(|start| {
                ...
                output[val_start..]
                    .find(suffix)
                    .map(|end| output[val_start..val_start + end].to_string())
            })
            .unwrap_or_default()
    }
    ...
    if !ak.is_empty() {
        result.insert("ANTHROPIC_API_KEY".to_string(), ak);
    }
    if !at.is_empty() {
        result.insert("ANTHROPIC_AUTH_TOKEN".to_string(), at);
    }
    if !bu.is_empty() {
        result.insert("ANTHROPIC_BASE_URL".to_string(), bu);
    }
...
fn read_claude_telemetry_usage_for_home(
    home: &Path,
    days: i64,
    force_refresh: bool,
    cache_only: bool,
    ...
    let latest_mtime_ms = telemetry_files
        .iter()
        .map(|path| file_modified_ms(path))
        .max()
        .unwrap_or(0);
    if !force_refresh {
        if let Some(payload) = read_claude_usage_cache(
            &cache_path,
            &telemetry_root,
            days.clamp(1, 366),
            file_count,
    ...
    for file_path in telemetry_files {
        ...
        let session_id = file_path
            ...
            .unwrap_or("")
            .to_string();
        let mut usage_entries: BTreeMap<
            String,
            (
        ...
        for line in reader.lines().map_while(Result::ok) {
            ...
            let Some(msg) = record.get("message").and_then(Value::as_object) else {
                continue;
            };
            let Some(usage) = msg.get("usage").and_then(Value::as_object) else {
                continue;
            };
            let parsed_ts = record
                .get("timestamp")
                .and_then(Value::as_str)
                .and_then(|value| chrono::DateTime::parse_from_rfc3339(value).ok())
                .map(|value| value.with_timezone(&chrono::Utc));
            let Some(ts_utc) = parsed_ts else {
                continue;
            };
            if ts_utc < cutoff {
                continue;
            ...
            let replace = usage_entries
                .get(&usage_key)
                .map(|entry| total >= entry.6)
                .unwrap_or(true);
            if replace {
        ...
        for (_key, (ts_utc, model, input, output, cache_read, cache_creation, _total)) in
            usage_entries
        {
            last_window_ts = Some(
                last_window_ts
                    .map(|prev| prev.max(ts_utc))
                    .unwrap_or(ts_utc),
            );
            session_input += input;
            ...
            bucket["input"] =
                json!(bucket.get("input").and_then(Value::as_u64).unwrap_or(0) + input);
            bucket["output"] =
                json!(bucket.get("output").and_then(Value::as_u64).unwrap_or(0) + output);
            bucket["cacheCreation"] = json!(
                bucket
                    .get("cacheCreation")
                    .and_then(Value::as_u64)
                    .unwrap_or(0)
            ...
            bucket["cacheRead"] =
                json!(bucket.get("cacheRead").and_then(Value::as_u64).unwrap_or(0) + cache_read);
            bucket["total"] = json!(
                bucket.get("total").and_then(Value::as_u64).unwrap_or(0)
                    + input
                    + output
                    + cache_read
                    + cache_creation
            ...
            bucket["cost"] = json!(
                bucket.get("cost").and_then(Value::as_f64).unwrap_or(0.0)
                    + calc_cost(&model, input, output, cache_read, cache_creation)
            );
            if !model.is_empty() && !model.starts_with('<') {
                let day_models = daily_model_tokens_map.entry(date).or_default();
                *day_models.entry(model).or_insert(0) +=
                    input + output + cache_read + cache_creation;
            }
        }

        let Some(last_window_ts) = last_window_ts else {
            continue;
        };
        if session_input == 0
            && session_output == 0
    ...
    let mut models_from_telemetry: Vec<Value> = model_map
        .into_iter()
        .map(|(model, bucket)| {
    ...
    let mut daily_model_tokens: Vec<Value> = daily_model_tokens_map
        .into_iter()
    ...
    let models = if !stats_models.is_empty() {
        stats_models
    } else {
        models_from_telemetry
    };
...
fn merge_claude_usage_payloads(days: i64, payloads: Vec<Value>, source_label: &str) -> Value {
    ...
    for payload in payloads {
        ...
        if let Some(entries) = payload.get("daily").and_then(Value::as_array) {
            for entry in entries {
                ...
                let bucket = daily_map.entry(date.clone()).or_insert_with(|| {
                    json!({
                    "date": date,
                    "input": 0_u64,
                    "output": 0_u64,
                ...
                bucket["input"] = json!(
                    bucket.get("input").and_then(Value::as_u64).unwrap_or(0)
                        + entry.get("input").and_then(Value::as_u64).unwrap_or(0)
                );
                bucket["output"] = json!(
                    bucket.get("output").and_then(Value::as_u64).unwrap_or(0)
                        + entry.get("output").and_then(Value::as_u64).unwrap_or(0)
                );
                bucket["cacheCreation"] = json!(
                    bucket
                        .get("cacheCreation")
                ...
                bucket["cacheRead"] = json!(
                    bucket.get("cacheRead").and_then(Value::as_u64).unwrap_or(0)
                        + entry.get("cacheRead").and_then(Value::as_u64).unwrap_or(0)
                );
                bucket["total"] = json!(
                    bucket.get("total").and_then(Value::as_u64).unwrap_or(0)
                        + entry.get("total").and_then(Value::as_u64).unwrap_or(0)
                );
                bucket["cost"] = json!(
                    bucket.get("cost").and_then(Value::as_f64).unwrap_or(0.0)
        ...
        if let Some(entries) = payload.get("models").and_then(Value::as_array) {
            for entry in entries {
                ...
                let bucket = model_map.entry(model.clone()).or_insert_with(|| {
                    json!({
                    "model": model,
                    "totals": {
                      "input": 0_u64,
                ...
                bucket["totals"]["input"] = json!(
                    bucket["totals"]
                        .get("input")
                        .and_then(Value::as_u64)
                        .unwrap_or(0)
                        + src_totals.get("input").and_then(Value::as_u64).unwrap_or(0)
                );
                bucket["totals"]["output"] = json!(
                    bucket["totals"]
                        .get("output")
                        .and_then(Value::as_u64)
                        .unwrap_or(0)
                ...
                bucket["totals"]["cacheCreation"] = json!(
                    bucket["totals"]
                        .get("cacheCreation")
                        .and_then(Value::as_u64)
                        .unwrap_or(0)
                ...
                bucket["totals"]["total"] = json!(
                    bucket["totals"]
                        .get("total")
                        .and_then(Value::as_u64)
                        .unwrap_or(0)
                        + src_totals.get("total").and_then(Value::as_u64).unwrap_or(0)
                );
                bucket["totals"]["cost"] = json!(
                    bucket["totals"]
                        .get("cost")
                        .and_then(Value::as_f64)
                        .unwrap_or(0.0)
        ...
        if let Some(entries) = payload.get("officialModels").and_then(Value::as_array) {
            for entry in entries {
                ...
                let bucket = official_model_map.entry(model.clone()).or_insert_with(|| {
                    json!({
                    "model": model,
                    "costUSD": 0.0_f64,
                    "inputTokens": 0_u64,
                ...
                bucket["costUSD"] = json!(
                    bucket.get("costUSD").and_then(Value::as_f64).unwrap_or(0.0)
                        + entry.get("costUSD").and_then(Value::as_f64).unwrap_or(0.0)
                );
                bucket["inputTokens"] = json!(
                    bucket
                        .get("inputTokens")
                        .and_then(Value::as_u64)
                        .unwrap_or(0)
                ...
                bucket["cacheReadInputTokens"] = json!(
                    bucket
                        .get("cacheReadInputTokens")
                        .and_then(Value::as_u64)
                        .unwrap_or(0)
                ...
                bucket["cacheCreationInputTokens"] = json!(
                    bucket
                        .get("cacheCreationInputTokens")
                        .and_then(Value::as_u64)
                        .unwrap_or(0)
    ...
    let daily_model_tokens = daily_model_tokens_map
        .into_iter()
        .map(|(date, tokens_by_model)| json!({ "date": date, "tokensByModel": tokens_by_model }))
        .collect::<Vec<_>>();
    ...
    official_models.sort_by(|left, right| {
        let left_total = left.get("inputTokens").and_then(Value::as_u64).unwrap_or(0)
            + left
                .get("outputTokens")
                .and_then(Value::as_u64)
                .unwrap_or(0)
...
fn read_claude_telemetry_usage(days: i64, force_refresh: bool, cache_only: bool) -> Value {
    let home = match claude_code_home() {
        Ok(path) => path,
        Err(_) => {
            return json!({"days": days, "generatedAt": chrono::Utc::now().to_rfc3339(), "source": "", "totals": {"input":0,"output":0,"cacheCreation":0,"cacheRead":0,"total":0,"cost":0.0}, "daily": [], "sessions": [], "models": [], "dailyModelTokens": []})
        }
    };
...
pub(crate) fn load_claudecode_state(query: &Value) -> Result<Value, String> {
    let query_object = parse_json_object(query);
    let force_usage_refresh = matches!(
        get_string(&query_object, "forceUsageRefresh").as_str(),
        "1" | "true" | "yes"
    );
    let cache_only = matches!(
        get_string(&query_object, "cacheOnly").as_str(),
        "1" | "true" | "yes"
    );
    ...
    let model = settings
        ...
        .unwrap_or("")
        .to_string();
    let always_thinking = settings
        .get("alwaysThinkingEnabled")
        .and_then(Value::as_bool)
        .unwrap_or(false);
    let skip_dangerous = settings
    ...
    fn pick_env(
        ...
        let from_settings = settings_env
            .get(var)
            .and_then(Value::as_str)
            .unwrap_or("")
            .to_string();
    ...
    let (auth_token_source, auth_token_value) =
        pick_env("ANTHROPIC_AUTH_TOKEN", &settings_env, &shell_env);
    let (base_url_source, base_url_value) =
        pick_env("ANTHROPIC_BASE_URL", &settings_env, &shell_env);
    ...
    let has_completed_onboarding = claude_json
    ...
    let active_profile_id = profiles_state
        .get("active")
        .and_then(Value::as_str)
        .unwrap_or("")
        .to_string();
    let default_plan = profiles_state
        ...
        .cloned()
        .unwrap_or_else(|| json!({}));
    let default_email = default_plan
        .get("email")
        .and_then(Value::as_str)
        .unwrap_or("")
        .trim()
        .to_string();
    let default_org = default_plan
        .get("organizationName")
        .and_then(Value::as_str)
        .unwrap_or("")
        .trim()
        .to_string();
    let default_plan_label = default_plan
        .get("plan")
        .and_then(Value::as_str)
        .unwrap_or("")
        .trim()
    ...
    let default_scope_visible = default_claude_json
        .get("oauthAccount")
        .and_then(Value::as_object)
        .is_some()
    ...
    if let Some(profiles) = profiles_state.get("profiles").and_then(Value::as_array) {
        for profile in profiles {
            ...
            let is_stale = profile
                ...
                .and_then(Value::as_bool)
                .unwrap_or(false);
            let include = (has_tokens
                || has_claude_usage_artifacts(&PathBuf::from(&config_dir))
    ...
    if matches!(
        login_info.get("method").and_then(Value::as_str),
        Some("oauth")
    ) {
    ...
    let scope_exists = |scope_id: &str| {
        available_scopes
            .iter()
            .any(|scope| scope.get("scopeId").and_then(Value::as_str).unwrap_or("") == scope_id)
    };
    ...
    let usage = if effective_scope_id == "all" {
        ...
        let payloads = homes
            .iter()
            .map(|scope_home| {
                read_claude_telemetry_usage_for_home(
                    scope_home,
                    usage_days,
                    force_usage_refresh,
                    cache_only,
        ...
        let scope_home = available_scopes
            .iter()
            .find(|scope| {
                scope.get("scopeId").and_then(Value::as_str).unwrap_or("") == effective_scope_id
            })
...
pub(crate) async fn model_eval_claudecode_provider(body: &Value) -> Result<Value, String> {
    ...
    if configured.is_null()
        ...
        && !(provider_key == "official"
            && (env_base_url.is_empty() || env_base_url.contains("anthropic.com")))
    {
        return Err(format!("未找到 Claude Code Provider：{provider_key}"));
    }

    let use_env_fallback = configured.is_null()
...
fn launch_terminal_command(
    ...
    let terminals = vec![
        (
            ...
            vec![
                "-e".to_string(),
                format!("bash -lc \"cd '{}' && {}\"", cwd_text, command_text),
            ],
        ),
        (
            "gnome-terminal",
            vec![
                ...
                "bash".to_string(),
                "-lc".to_string(),
                format!("cd '{}' && {}", cwd_text, command_text),
            ],
        ),
        (
            "konsole",
...
fn build_windows_binary_command(
    binary_path: &str,
    args: &[String],
    fallback_binary: &str,
) -> String {
...
fn inspect_openclaw_port_occupants(port: &str) -> Vec<Value> {
    ...
    let output = match create_command("netstat")
        .args(["-ano", "-p", "tcp"])
        .output()
    {
        Ok(out) => out,
    ...
    for line in stdout.lines() {
        ...
        if let Ok(ps_out) = create_command("powershell.exe")
        ...
        if name == "未知进程" {
            if let Ok(task_out) = create_command("tasklist")
                .args(["/FI", &format!("PID eq {}", pid), "/FO", "CSV", "/NH"])
                .output()
            {
                if let Some(first_line) = String::from_utf8_lossy(&task_out.stdout)
                    .lines()
                    .find(|line| !line.trim().is_empty() && !line.trim().starts_with("INFO:"))
                {
        ...
        let likely_openclaw = format!("{} {}", name, command_line)
            .to_lowercase()
            .contains("openclaw");
        items.push(json!({
...
fn extract_url_from_text(text: &str) -> Option<String> {
    text.split_whitespace()
        ...
        .map(|part| {
            part.trim_end_matches(|ch: char| [')', ',', '.', ';'].contains(&ch))
                .to_string()
        })
}
...
fn extract_openclaw_gateway_token(text: &str) -> Option<String> {
    text.split_whitespace()
        ...
        .map(|part| {
            part.trim_matches(|ch: char| !ch.is_ascii_alphanumeric() && ch != '_')
                .to_string()
        })
...
fn normalize_openclaw_dashboard_bootstrap_url(raw_url: &str, gateway_token: &str) -> String {
    ...
    let mut url = reqwest::Url::parse(input)
        .unwrap_or_else(|_| reqwest::Url::parse("http://127.0.0.1:18789/").unwrap());
    url.set_fragment(None);
    if !gateway_token.trim().is_empty() {
        url.query_pairs_mut()
            .clear()
            .append_pair("token", gateway_token)
            .finish();
    }
    url.to_string()
}
...
fn resolve_claude_config_dir(object: &serde_json::Map<String, Value>) -> Option<PathBuf> {
    ...
    if !profile_id.is_empty() {
        let dir = app_home()
            .ok()?
            .join("claudecode-oauth-profiles")
            .join(&profile_id);
        if dir.exists() {
            return Some(dir);
        }
...
pub(crate) fn launch_claudecode(body: &Value) -> Result<Value, String> {
    ...
    if config_dir.is_none() {
        ...
        return Ok(
            json!({ "ok": true, "cwd": cwd.to_string_lossy().to_string(), "message": message }),
        );
    }
...
fn normalize_opencode_expiry(value: &Value) -> String {
    ...
    let Some(raw_number) = seconds else {
        return String::new();
    };
...
fn find_opencode_auth_entry(
    ...
    auth_entries
        .iter()
        .find(|entry| {
            let auth_key = normalize_opencode_auth_entry_key(
...
fn open_code_remove_binary_action() -> Result<Value, String> {
    ...
    if cfg!(target_os = "windows") {
        return open_code_shell_action(&format!(
            "Remove-Item -Force '{}'",
            path.replace('\'', "''")
        ));
    }
...
fn run_opencode_action(kind: &str, body: &Value) -> Result<Value, String> {
    ...
    let mut result = match kind {
        ...
        "reinstall" => match effective_method.as_str() {
            ...
            "choco" => {
                open_code_shell_action("choco uninstall opencode -y; choco install opencode -y")?
            }
            _ => open_code_shell_action(OPENCODE_INSTALL_SCRIPT_UNIX)?,
        },
        ...
        "uninstall" => match effective_method.as_str() {
            "domestic" | "npm" => open_code_npm_action(
                vec![
                    "uninstall".to_string(),
                    "-g".to_string(),
                    OPENCODE_PACKAGE.to_string(),
    ...
    if let Some(result_object) = result.as_object_mut() {
        ...
        result_object.insert(
            "official".to_string(),
            json!(effective_method != "domestic"),
        );
...
pub(crate) fn load_opencode_state(query: &Value) -> Result<Value, String> {
    ...
    let active_provider = providers
        .iter()
        .find(|item| {
            item.get("key").and_then(Value::as_str) == Some(selected_provider_key.as_str())
        })
        .cloned();
...
fn parse_hermes_env(content: &str) -> BTreeMap<String, String> {
...
fn with_hermes_launch_env(command_text: &str, env: &BTreeMap<String, String>) -> String {
    if env.is_empty() {
        return command_text.to_string();
    }
    if cfg!(target_os = "windows") {
        let prefix = env
            ...
            .collect::<String>();
        return format!("{}{}", prefix, command_text);
    }
    let exports = env
        .iter()
        .map(|(key, value)| format!("{}={}", key, quote_posix_shell_arg(value)))
        .collect::<Vec<_>>()
        .join(" ");
    format!("export {}; {}", exports, command_text)
}

pub(crate) fn launch_hermes(body: &Value) -> Result<Value, String> {
    let object = parse_json_object(body);
    let cwd = {
        let input = get_string(&object, "cwd");
        expand_home_path(&input).map_or_else(home_dir, Ok)?
    };
    let binary = find_tool_binary("hermes");
    if !binary
        .get("installed")
        .and_then(Value::as_bool)
        .unwrap_or(false)
    {
        return Err(
            "Hermes Agent 尚未安装，请先按官方方式安装并确保 hermes 命令在 PATH 中".to_string(),
        );
    }
    let binary_path = binary
        .get("path")
        ...
        .unwrap_or("hermes");
    let command = if cfg!(target_os = "windows") {
        build_windows_binary_command(binary_path, &[], "hermes")
    } else {
        quote_posix_shell_arg(binary_path)
    };
    let hermes_home = home_dir()?.join(".hermes");
    let launch_env = parse_hermes_env(&read_text(&hermes_home.join(".env"))?);
    let command = with_hermes_launch_env(&command, &launch_env);
    let message = launch_terminal_command(&cwd, &command, "Hermes Agent")?;
    Ok(json!({
      "ok": true,
      "cwd": cwd.to_string_lossy().to_string(),
      "message": message,
      "envInjected": launch_env.keys().cloned().collect::<Vec<_>>()
    }))
}

pub(crate) fn launch_gemini(body: &Value) -> Result<Value, String> {
    let object = parse_json_object(body);
    let cwd = {
        let input = get_string(&object, "cwd");
        expand_home_path(&input).map_or_else(home_dir, Ok)?
    };
    let binary = find_tool_binary("gemini");
    if !binary
        .get("installed")
        .and_then(Value::as_bool)
        .unwrap_or(false)
    {
        return Err(
            "Gemini CLI 尚未安装，请先安装 @google/gemini-cli 并确保 gemini 命令在 PATH 中"
                .to_string(),
        );
    }
    let binary_path = binary
        ...
        .filter(|text| !text.trim().is_empty())
        .unwrap_or("gemini");
    let message = launch_terminal_for_tool(&cwd, binary_path, "Gemini CLI", "gemini")?;
    Ok(json!({ "ok": true, "cwd": cwd.to_string_lossy().to_string(), "message": message }))
}

fn claude_desktop_windows_candidates() -> Vec<PathBuf> {
    let mut candidates = Vec::new();
    if let Ok(local_app_data) = std::env::var("LOCALAPPDATA") {
        candidates.push(
            PathBuf::from(local_app_data)
...
pub(crate) fn launch_claude_desktop(body: &Value) -> Result<Value, String> {
    let object = parse_json_object(body);
    let cwd = {
        let input = get_string(&object, "cwd");
        expand_home_path(&input).map_or_else(home_dir, Ok)?
    ...
    if cfg!(target_os = "macos") {
        let output = create_command("open")
            .args(["-a", "Claude"])
            .output()
            .map_err(|error| error.to_string())?;
        if !output.status.success() {
            ...
            return Err(if detail.is_empty() {
                "Claude Desktop 尚未安装，或 macOS 无法通过 open -a Claude 打开".to_string()
            } else {
                detail
            });
        }
        return Ok(json!({
          "ok": true,
          "cwd": cwd.to_string_lossy().to_string(),
          "message": "Claude Desktop 已打开",
          "method": "open -a Claude"
        }));
    }
    ...
    if cfg!(target_os = "windows") {
        ...
        let binary_path = detected
            .or_else(|| {
                claude_desktop_windows_candidates()
                    .into_iter()
                    .find(|path| path.exists())
            })
            .ok_or_else(|| {
                "Claude Desktop 尚未安装，或未在常见安装路径中检测到 Claude.exe".to_string()
            })?;
        let binary_text = binary_path.to_string_lossy().to_string();
        create_command(&binary_text)
            .current_dir(&cwd)
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
    ...
    let binary_path = command_exists("claude-desktop").ok_or_else(|| {
        "当前平台未检测到 Claude Desktop 启动入口；请手动打开 Claude Desktop".to_string()
    })?;
    create_command(&binary_path)
        .current_dir(&cwd)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
        .stderr(Stdio::null())
...
pub(crate) fn load_openclaw_state() -> Result<Value, String> {
    ...
    let gateway_token_cfg = config
        .pointer("/gateway/auth/token")
        .and_then(Value::as_str)
        .unwrap_or("")
        .to_string();
    let gateway_token = if !gateway_token_env.is_empty() {
        gateway_token_env
    } else {
        gateway_token_cfg
    };
    ...
    let gateway_port_cfg = config
        .pointer("/gateway/port")
        .and_then(Value::as_u64)
        .map(|p| p.to_string())
        .unwrap_or_default();
    let gateway_port = if !gateway_port_env.is_empty() {
        gateway_port_env
    } else if !gateway_port_cfg.is_empty() {
        gateway_port_cfg
    } else {
        "18789".to_string()
    };
...
fn build_openclaw_dashboard_url(gateway_url: &str, config: &Value, gateway_token: &str) -> String {
    let base = config
        .pointer("/gateway/controlUi/basePath")
        .and_then(Value::as_str)
        .unwrap_or("/");
...
pub(crate) fn get_openclaw_dashboard_url(body: &Value) -> Result<Value, String> {
    ...
    let token = state
        ...
        .and_then(Value::as_str)
        .unwrap_or("");
    let url = normalize_openclaw_dashboard_bootstrap_url(
        &extract_url_from_text(&merged).unwrap_or_else(|| fallback.to_string()),
        token,
    );
    Ok(
        json!({ "ok": !url.is_empty(), "url": url, "stdout": stdout.trim(), "stderr": stderr.trim(), "command": format!("{} dashboard --no-open", bin_path) }),
    )
}
...
pub(crate) fn repair_openclaw_dashboard_auth(body: &Value) -> Result<Value, String> {
    ...
    let gateway_auth_mode = state
        .get("gatewayAuthMode")
        .and_then(Value::as_str)
        .unwrap_or("token")
        .to_string();
    let mut gateway_token = state
        .get("gatewayToken")
        .and_then(Value::as_str)
        .unwrap_or("")
        .to_string();
    ...
    let dashboard =
        get_openclaw_dashboard_url(&json!({ "cwd": cwd.to_string_lossy().to_string() }))?;
...
pub(crate) fn launch_openclaw(body: &Value) -> Result<Value, String> {
        ...
        .unwrap_or(false)
    {
        ...
        response.insert(
            ...
            state.get("gatewayUrl").cloned().unwrap_or(Value::Null),
        );
    ...
    let command = if cfg!(target_os = "windows") {
        build_windows_binary_command(
            bin_path,
            &["gateway".to_string(), "--force".to_string()],
            "openclaw",
        )
    } else {
...
pub(crate) fn onboard_openclaw(body: &Value) -> Result<Value, String> {
    ...
    let success =
        stdout.contains("Updated") || stdout.contains("openclaw.json") || output.status.success();
...
fn run_remote_ssh_command(
    ...
    if auth_method == "password" {
        if command_exists("sshpass").is_none() {
            return Err(
        "密码登录需要本机安装 sshpass（macOS 可用 brew install hudochenkov/sshpass/sshpass）"
          .to_string(),
      );
        }
        let mut args = vec!["-e".to_string(), "ssh".to_string()];
        args.extend(ssh_args);
        return run_command_dynamic(
            "sshpass",
            &args,
            None,
            Some(("SSHPASS", password.to_string())),
        );
    }
...
pub(crate) fn install_openclaw_remote(body: &Value) -> Result<Value, String> {
    ...
    let key_path = if key_path_raw.starts_with("~/") {
        if let Some(home) = dirs::home_dir() {
            home.join(key_path_raw.trim_start_matches("~/"))
                .to_string_lossy()
                .to_string()
        } else {
            key_path_raw
    ...
    let verify_stdout = verify_result
        .get("stdout")
        .and_then(Value::as_str)
        .unwrap_or("")
        .to_string();
    let verify_stderr = verify_result
        .get("stderr")
        .and_then(Value::as_str)
        .unwrap_or("")
        .to_string();
...
pub(crate) fn stop_openclaw_gateway() -> Result<Value, String> {
    ...
    if cfg!(target_os = "windows") {
        let netstat_output = create_command("netstat")
            .args(["-ano", "-p", "tcp"])
            .output();
        if let Ok(output) = netstat_output {
            ...
            for line in stdout.lines() {
                ...
                let local_addr = parts[1];
                let pid = parts[4];
                if !local_addr.ends_with(&format!(":{}", gateway_port))
                    || !seen.insert(pid.to_string())
                {
                    continue;
                }
                if create_command("taskkill")
                    .args(["/F", "/T", "/PID", pid])
                    .output()
    ...
    if !after
        ...
        && !after
            ...
            .unwrap_or(false)
    {
        let method_text = if methods.is_empty() {
            "none".to_string()
        } else {
            methods.join(" -> ")
        };
...
pub(crate) fn kill_openclaw_port_occupants(body: &Value) -> Result<Value, String> {
    ...
    let state = load_openclaw_state()?;
    let occupants = state
        .get("gatewayPortOccupants")
        .and_then(Value::as_array)
        .cloned()
        .unwrap_or_default();
    let selected = occupants
        .into_iter()
        .filter(|item| {
