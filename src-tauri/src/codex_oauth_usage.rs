...
fn build_headers(account: &OAuthAccount, token_kind: &str) -> Result<HeaderMap, String> {
    ...
    let token = if token_kind == "id_token" {
        &account.id_token
    } else {
    ...
    headers.insert(
        ...
        HeaderValue::from_static(CODEX_DEFAULT_VERSION),
    );
...
fn should_retry_chatgpt_dns_fallback(outcome: &UsageProbeOutcome) -> bool {
    outcome.saw_network_error
        && outcome.first_json_payload.is_none()
        && !outcome.saw_auth_error
        && !outcome.saw_blocked
}
...
fn chatgpt_dns_fallback_reason_for_addrs(addrs: &[IpAddr]) -> Option<String> {
    ...
    let sample = addrs
        ...
        .join(", ");
    Some(format!(
        "系统 DNS 将 chatgpt.com 解析到非 Cloudflare 地址: {sample}"
    ))
}
...
async fn chatgpt_dns_fallback_reason() -> Option<String> {
    ...
    match result {
        ...
        Err(_) => Some(format!(
            "系统 DNS 解析 chatgpt.com 超过 {CODEX_SYSTEM_DNS_PREFLIGHT_MS}ms"
        )),
    }
}

fn persist_refreshed_oauth_tokens(
    codex_home: &PathBuf,
    tokens: &RefreshedOAuthTokens,
) -> Result<(), String> {
    let auth_path = codex_home.join(AUTH_FILENAME);
    let raw = read_text(&auth_path)?;
...
async fn refresh_codex_oauth_tokens(
    client: &Client,
    refresh_token: &str,
) -> Result<RefreshedOAuthTokens, String> {
...
fn value_as_f64(value: &Value) -> Option<f64> {
    ...
    let cleaned = text.replace(',', "").replace('%', "").trim().to_string();
...
fn window_from_map(map: &Map<String, Value>, path: &str) -> Option<Value> {
    ...
    let limit_window_seconds = map_number(
        map,
        &[
...
fn min_remaining_percent(windows: &[Value]) -> Option<f64> {
    windows
        ...
        .fold(None, |acc: Option<f64>, value| {
            Some(acc.map_or(value, |prev| prev.min(value)))
        })
}
...
fn normalize_usage_payload(
    ...
    let total_tokens = find_number(
        payload,
        &["total_tokens", "totalTokens", "tokens", "token_count"],
    );
    let input_tokens = find_number(
        payload,
        &[
            "input_tokens",
            "inputTokens",
    ...
    let payload_account_id =
        find_string(payload, &["account_id", "accountId", "chatgpt_account_id"]);
    let payload_plan = find_string(
        payload,
        &["plan_type", "planType", "plan", "tier", "subscription_plan"],
    );
    let membership_expires_at = find_timestamp(
        payload,
        &[
    ...
    let membership_renews_at = find_timestamp(
        payload,
        &[
            "renews_at",
            "renew_at",
    ...
    let credits_available = find_number(
        payload,
        &["available_count", "available_credits", "credits_available"],
    );
...
async fn run_codex_oauth_usage_probe(
    ...
    while index < endpoint_specs.len() {
        ...
        if status_code == 401 || status_code == 403 {
            ...
            if spec.token_kind == "access_token"
                ...
                && !account.refresh_token.trim().is_empty()
            {
                attempts.push(endpoint_attempt(
                    ...
                    if body_message.is_empty() {
                        "access_token 鉴权失败，尝试刷新 OAuth token"
                    } else {
                        &body_message
                    },
                ));
...
pub(crate) async fn query_codex_oauth_usage(body: &Value) -> Result<Value, String> {
    ...
    if let Some(reason) = chatgpt_dns_fallback_reason().await {
        ...
        outcome =
            run_codex_oauth_usage_probe(&fallback_client, &mut account, &codex_home, &mut attempts)
                .await?;
    } else {
        let client = build_oauth_usage_client(system_timeout_ms, false)?;
        outcome =
            run_codex_oauth_usage_probe(&client, &mut account, &codex_home, &mut attempts).await?;
    }
    ...
    if let Some(payload) = outcome.first_json_payload {
        return Ok(normalize_usage_payload(
            &payload,
            &account,
            attempts,
            &codex_home,
...
mod tests {
    ...
    fn collect_windows_parses_wham_used_percent() {
        ...
        assert_eq!(
            five_hour.get("remainingPercent").and_then(Value::as_f64),
            Some(99.0)
        );
        assert_eq!(
            five_hour.get("usedPercent").and_then(Value::as_f64),
            Some(1.0)
        );
        assert_eq!(
            five_hour.get("resetInSeconds").and_then(Value::as_f64),
            Some(18000.0)
        );
        assert_eq!(
        ...
        assert_eq!(
            five_hour.get("limitWindowSeconds").and_then(Value::as_f64),
            Some(18000.0)
        );
        ...
        assert_eq!(
            weekly.get("remainingPercent").and_then(Value::as_f64),
            Some(100.0)
        );
        assert_eq!(weekly.get("usedPercent").and_then(Value::as_f64), Some(0.0));
        assert_eq!(
            weekly.get("resetInSeconds").and_then(Value::as_f64),
            Some(604800.0)
        );
        assert_eq!(
