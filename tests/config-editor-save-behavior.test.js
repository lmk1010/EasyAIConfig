...
test('provider router is a standalone local API-key gateway page', () => {
  ...
  assert.match(appJs, /pd-router-log-chart/);
  assert.match(appJs, /请求趋势/);
  assert.match(appJs, /<span>输入<\/span>/);
  assert.match(appJs, /<span>缓存<\/span>/);
  assert.match(appJs, /<span>输出<\/span>/);
  assert.match(appJs, /<span>耗时 \/ TTFT<\/span>/);
  assert.doesNotMatch(appJs, /暂无 Provider 统计/);
  ...
  assert.match(routerRenderBody, /data-provider-router-power/);
  assert.match(routerRenderBody, /pd-router-protocol-paths/);
  assert.match(routerRenderBody, /\/v1\/responses/);
  ...
  assert.match(appJs, /state\.providerRouter\.activeTab = 'gateway'/);
  ...
  assert.match(stylesCss, /\.pd-router-log-chart/);
  assert.match(stylesCss, /grid-template-columns:\s*104px minmax\(220px, 1\.5fr\)/);
  assert.match(stylesCss, /\.page-view\[data-page="providerRouter"\][\s\S]{0,140}overflow:\s*hidden/);
  assert.match(stylesCss, /\.provider-router-page[\s\S]{0,220}overflow-y:\s*auto/);
  assert.match(stylesCss, /\.provider-router-page::-webkit-scrollbar/);
  ...
  assert.doesNotMatch(routerRenderBody, /key: 'rectifier'/);
  assert.doesNotMatch(routerRenderBody, /key: 'stats'/);
