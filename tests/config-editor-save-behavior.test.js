...
test('provider detail usage aggregates cost by model with time and model filters', () => {
  ...
  assert.match(usageBody, /codexMetrics\.providerModels/);
  assert.match(usageBody, /matched\?\.requests/);
  assert.doesNotMatch(usageBody, /matchedSessions\.length\s*\|\|\s*Number\(matched\?\.events/);
  ...
  assert.match(costTableBody, /请求次数/);
...
test('provider router is a standalone local API-key gateway page', () => {
  ...
  assert.match(routerRenderBody, /pd-router-console/);
  assert.match(routerRenderBody, /pd-router-console-bar/);
  assert.match(routerRenderBody, /pd-router-console-routes/);
  assert.match(routerRenderBody, /data-provider-router-quick-start/);
  assert.match(routerRenderBody, /接入并启动/);
  assert.match(routerRenderBody, /pd-router-more-menu/);
  assert.match(routerRenderBody, /data-provider-router-tool-select/);
  assert.match(routerRenderBody, /data-provider-router-primary-select/);
  assert.match(routerRenderBody, /pd-router-strategy-content/);
  assert.match(routerRenderBody, /pd-router-clients-content/);
  assert.match(routerRenderBody, /pd-router-runtime-content/);
  ...
  assert.match(routerRenderBody, /key: 'routes'/);
  assert.match(routerRenderBody, /key: 'strategy'/);
  assert.match(routerRenderBody, /key: 'clients'/);
  assert.match(routerRenderBody, /key: 'runtime'/);
  assert.match(routerRenderBody, /key: 'rectifier'/);
  assert.match(routerRenderBody, /key: 'stats'/);
  assert.match(routerEventsBody, /\['routes', 'strategy', 'clients', 'runtime', 'stats', 'rectifier'\]/);
  assert.match(routerRenderBody, /label: '线路'/);
  assert.match(routerRenderBody, /label: '策略'/);
  assert.match(routerRenderBody, /label: '客户端'/);
  assert.match(routerRenderBody, /label: '运行'/);
  assert.match(routerRenderBody, /格式转换/);
  assert.doesNotMatch(routerRenderBody, /label: '客户端接入'/);
  assert.doesNotMatch(routerRenderBody, /label: '网关运行'/);
  assert.doesNotMatch(routerRenderBody, /label: '负载池'/);
  ...
  assert.match(stylesCss, /\.pd-router-console-bar/);
  assert.match(stylesCss, /\.pd-router-more-menu/);
  assert.match(stylesCss, /\.pd-router-simple-provider/);
  assert.match(stylesCss, /grid-template-columns: repeat\(6, minmax\(104px, 1fr\)\)/);
  assert.match(stylesCss, /\.provider-router-page \{[\s\S]{0,160}max-width: none/);
  assert.doesNotMatch(routerRenderBody, /pd-router-advanced/);
  assert.doesNotMatch(routerRenderBody, /pd-router-simple-step/);
  assert.doesNotMatch(routerRenderBody, /3 步完成/);
...
test('provider model support exposes Codex model catalog synchronization on both runtimes', () => {
  const serverJs = readFileSync(new URL('../src/server.js', import.meta.url), 'utf8');
  const routesRs = readFileSync(new URL('../src-tauri/src/routes.rs', import.meta.url), 'utf8');
